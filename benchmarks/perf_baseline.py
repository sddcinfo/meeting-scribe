...
async def probe_scribe_status(_client: httpx.AsyncClient) -> dict:
    ...
    # Falls back to {} if the server admin gate redirects us to /auth — the
    # rest of the harness doesn't depend on this metadata.
    import os

    port = int(os.environ.get("SCRIBE_PORT") or "443")
    ap_ip = os.environ.get("SCRIBE_AP_IP") or "10.42.0.1"
    try:
        async with httpx.AsyncClient(verify=_SSL_CTX, timeout=5) as c:
            r = await c.get(f"https://{ap_ip}:{port}/api/status")
