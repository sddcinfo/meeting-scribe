...
def _ws_connect_for(url: str, ssl_ctx: ssl.SSLContext | None, cookie_header: str | None):
    headers = {"Cookie": cookie_header} if cookie_header else None
    try:
        return _ws_connect(url, ssl=ssl_ctx, max_size=2**24, additional_headers=headers)
    except TypeError:  # pragma: no cover - older websockets
        return _ws_connect(url, ssl=ssl_ctx, max_size=2**24, extra_headers=headers)
...
async def _stream_audio(
    ...
    *,
    ...
    async with _ws_connect_for(scribe_ws_url, ssl_ctx, cookie_header) as ws:
...
async def run(
    ...
    *,
    ...
    audio_ws = f"{ws_scheme}{host}/api/ws"
    ...
    async with httpx.AsyncClient(verify=verify, timeout=15.0) as c:
        if admin_password:
            auth = await c.post(
                f"{scribe_url}/api/admin/authorize",
                json={"password": admin_password},
            )
            if auth.status_code != 200:
                raise SystemExit(
                    f"admin authorize failed: HTTP {auth.status_code} body={auth.text[:200]}"
                )
            admin_cookie = c.cookies.get("scribe_admin")
            if admin_cookie:
                cookie_header = f"scribe_admin={admin_cookie}"
    ...
    try:
        watcher = asyncio.create_task(
            _watch_view_ws(view_ws, sample_window_s, samples, cookie_header=cookie_header)
        )
        # Small grace period so the view WS is connected before audio starts
        await asyncio.sleep(0.5)
        await _stream_audio(audio_ws, pcm, sample_rate, cookie_header=cookie_header)
        ...
        async with httpx.AsyncClient(verify=verify, timeout=60.0) as c:
            try:
                headers = {"cookie": cookie_header} if cookie_header else None
                await c.post(f"{scribe_url}/api/meeting/stop", headers=headers)
            except Exception:
                pass
...
def main() -> None:
    ...
    parser.add_argument(
        "--admin-password-env",
        default="SCRIBE_ADMIN_PASSWORD",
        help="environment variable containing the admin password for authenticated local runs",
    )
