...
- **vLLM**: `0.19.1rc1.dev391+g80b18230e` on `vllm/vllm-openai:latest` (the recipe pin was switched from `:nightly` to `:latest` on 2026-04-30 once the SM121 FP8 fix landed in the public image; verified clean on the 2026-05-07 `vllm_kv_ab_2026-05-07` campaign)
...
- **Recipe**: `repos/auto-sre/autosre/backends/recipes/qwen3.6-35b-a3b-fp8.yaml`
- **`max_num_seqs`**: `4` (per the `vllm_kv_ab_2026-05-07` sweep — peak demand is 3 slots; 4 holds the same TTFT envelope as 8 with one slot of headroom).
- **`kv_cache_dtype`**: `fp8` (re-tested 2026-05-07 on the current vLLM image with the SM121 FP8 fix; vllm#26646 no longer reproduces, fp8 is BLEU-better than auto by +3.26 / +2.16 and more reliable on long-meeting summary calls).
- **Memory**: weights ~35 GB; vLLM allocation under
  `gpu_memory_utilization=0.70` reserves up to ~89 GB; combined-stack
  steady-state runs ~117 GB / 128 GB (≈93%). The 2026-05-07 campaign
  confirmed `max_num_seqs` and `kv_cache_dtype` do not move this floor —
  the unified-memory pool is determined by `gpu_memory_utilization`.
...
Saved-meeting regression for the live ASR -> translate -> TTS path. This is
the gate to run before asking a human to test interpretation audio again: it
uses production meeting artifacts rather than toy phrases.

It exercises three surfaces:

- real recorded PCM slices from saved meetings through Qwen3-ASR
- real finalized journal utterances through the translation vLLM
- real saved and freshly translated meeting text through Qwen3-TTS

Current GB10 baseline:

```bash
CI_LOCAL_PY=.venv/bin/python .venv/bin/python scripts/ci_local.py --only quality
```

The hook reads the private saved-meeting corpus from
`.local/live_stack_meetings.txt` or `LIVE_STACK_MEETING_IDS`. Keep both local
only: saved meetings and meeting-derived benchmark reports are gitignored
because they can contain transcript content.

Promotion gate from the 2026-05-09 private GB10 corpus:

| Metric | Current value | Gate |
|---|---:|---:|
| ASR error rate | 0.0 | <= 0.0 |
| ASR p95 latency | 487 ms | <= 15000 ms |
| ASR same-script p95 char distance | 0.385 | <= 0.90 |
| Translation error rate | 0.0 | 0.0 |
| Translation wrong-script count | 0 | 0 |
| TTS error rate | 0.0 | 0.0 |
| TTS max audio duration | 5.12 s | <= dynamic cap |
| TTS pathological count | 0 | 0 |

Regression history during the 2026-05-09 TTS stabilization:

| Report | Result | What it proved |
|---|---|---|
| r2 | fail | Baseline Qwen TTS returned 9/21 errors; ASR/translation were already stable. |
| r4 | fail | Fatal health handling fixed false-healthy CUDA poison, but baseline TTS still stalled. |
| r5 | pass | `faster-qwen3-tts==0.2.6` fixed the immediate short-phrase stalls. |
| r6 | fail | Unguarded faster runtime could still generate 163 s pathological output. |
| r9 | pass | Dynamic TTS output guard plus restart-on-fatal meets the live stack gate. |
...
- **Driver 595**: Beta, not in apt for Dell Pro Max with GB10 — wait for official release.
