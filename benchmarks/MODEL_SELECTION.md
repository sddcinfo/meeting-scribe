...
| `cyankiwi/Qwen3.6-35B-A3B-AWQ-4bit` / `QuantTrio/...-AWQ` | Dell Pro Max with GB10 forum testing rates AWQ inferior to FP8 on GB10 — *"cannot be recommended even for single Spark"*. |
...
- `VLLM_MARLIN_USE_ATOMIC_ADD=1` — Dell Pro Max with GB10 thread 366822 recommendation.
...
**Verdict**: Qwen3-TTS-12Hz-0.6B-Base remains the right choice **today** — low TTFA through the dedicated faster-qwen3-tts container, JA in the 10-language set, and no licensing ambiguity.  But the field tightened materially in March-April 2026:

**2026-05-09 GB10 runtime update**: production stays on
Qwen3-TTS-12Hz-0.6B-Base, but the runtime is now
`faster-qwen3-tts==0.2.6` on the pinned PyTorch 2.11 wheel stack with dynamic
output-duration guards. The old baseline backend stalled on short English
phrases; unguarded faster 0.2.6 could intermittently generate 163 s of audio.
The promoted configuration passed
`benchmarks/results/live_stack_poly_japanese_20260509_r9_dynamic_guard.json`
with 0 TTS errors and 0 pathological outputs. See `docs/gb10-live-stack.md`.
...
| 2026-04-30 | vLLM image | `vllm/vllm-openai:nightly` → `:latest` (supersedes 2026-04-18) | The `:nightly` pin was a defensive workaround while the SM121 FP8 fix was landing. Once it shipped in `:latest`, the pin was advanced to drop the moving-target risk; verified clean on the 2026-05-07 `vllm_kv_ab_2026-05-07` campaign (boots cleanly, zero engine errors across 6 perf runs, BLEU positive vs the prior baseline). |
| 2026-05-07 | vLLM config | `max_num_seqs` 8 → 4, `kv_cache_dtype` auto → fp8 (qwen3.6-35b-a3b-fp8) | `vllm_kv_ab_2026-05-07` showed peak demand under the perf-harness workload is 3 slots (`requests_running_peak=3.0` across all configs except `max_num_seqs=2`, which forced 117 preempts and 8× coding TTFT p95). 4 holds the same TTFT envelope as 8 with one slot of headroom. fp8 KV no longer reproduces vllm#26646 on `:latest` — boots cleanly, zero errors, +3.26/+2.16 BLEU vs `auto`, and reliably handles the long-meeting summary call that the `auto` path consistently times out on. Baseline: `gb10_qwen36_fp8_seq4_kv-fp8`. Full data: `benchmarks/results/vllm_kv_ab_2026-05-07/RESULT.md`. |
| 2026-05-09 | TTS runtime | `faster-qwen3-tts==0.2.6` + pinned PyTorch 2.11/CUDA 13 wheel stack + dynamic output guard | Saved-meeting live-stack regression moved from failing baseline TTS (`r2`: 42.9% TTS error rate; `r4`: 8.7% TTS error rate) to passing (`r9`: ASR/translation/TTS error rates all 0, TTS max 5.12 s, pathological count 0). Unguarded faster 0.2.6 was rejected after `r6` generated 163 s pathological audio; the dynamic guard mirrors the benchmark gate and forces a clean container restart on runaway synthesis. |
| 2026-05-14 | Audio pipeline | **REJECT NS-1 (server-side denoise) and VAD-1 (neural VAD swap) from the `soniqo/speech-android` survey** | Directional probes against the live `Qwen/Qwen3-ASR-1.7B` backend show the plan's premise was Whisper-era reasoning that doesn't carry. **NS-1**: spectral-gating denoise (noisereduce stand-in for DeepFilterNet3) replayed on 5 real meetings produced 0 transcript improvements / 3 active degradations across ~20 segments (e.g. meeting `4f8d6b69` segment `97ba9065`: CER 10.6 %→25.2 % under denoise; `39dd249b` flipped EN→ZH on the single word "Um."). Qwen3-ASR's training corpus already covers noisy meeting audio; preprocessing fights it. **VAD-1**: Silero v5 detection floor empirically sits at RMS≈0.003 vs. the current gate at 0.005 (~40 % softer, not the 70 % the plan presumed); real-meeting confusion matrix shows < 2 % of buffers change decision, mostly favorable (1–3 noise-only buffers/meeting rejected) but too small for a bootstrap-CI bench to converge against the engineering cost (new ORT runtime dep, per-stream state across 3 call sites, watchdog-interaction risk). Path forward: stay on the existing RMS gate and Qwen3-ASR's native noise tolerance; re-direct effort to **Sortformer** (NVIDIA end-to-end diarization, CUDA-native — targets `project_meeting_scribe_diarize_tuning` directly) or **Qwen3-ForcedAligner** (word-level timestamps for refinement-worker speaker attribution). Probes: `scripts/bench/probe_silero_envelope.py`, `scripts/bench/probe_denoise_asr.py` (both removed in the same change set per `feedback_no_backcompat_pristine`; see `/home/bradlay/.claude/plans/sharded-herding-flame.md` for the abandoned full plan + retained methodology). |
