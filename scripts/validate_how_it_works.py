...
def _recipe_memory_estimates() -> dict[str, float]:
    """Pick up explicit weight estimates from the recipes.

    The HTML resource table reports per-model **weights**; the recipe's
    ``gpu_memory_utilization`` is the **KV-cache reservation fraction**
    of the GPU memory pool, which is a different quantity (and the
    overlap is why the prior util×121 derivation produced false-positive
    drift findings against weight rows).

    The HTML totals line — "+ vLLM KV cache reservation at
    gpu_memory_utilization=0.70" — is where reservation belongs; the
    per-row check is weights-only.

    A recipe opts into the per-row check by adding a tagged comment::

        # weights: ~N GB

    Recipes without that tag are skipped here. Diarization (pyannote)
    isn't vLLM and already documents its weights in a similar inline
    form.
    """
    estimates: dict[str, float] = {}

    for name, content in _recipe_yamls().items():
        weights_match = re.search(r"#\s*weights:\s*~?(\d+(?:\.\d+)?)\s*GB", content, re.IGNORECASE)
        # Diarization comment lives on the gpu_memory_utilization line —
        # accept the legacy shape so we don't need to migrate sortformer.
        legacy_diariz = re.search(r"#\s*Small model,\s*~?(\d+(?:\.\d+)?)\s*GB", content)
        ...
        if weights_match:
            estimates[key] = float(weights_match.group(1))
        elif key == "Diarization" and legacy_diariz:
            estimates[key] = float(legacy_diariz.group(1))
