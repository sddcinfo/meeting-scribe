...
def _evaluate_pinned(model: _PinnedModel, api) -> _DriftRow:
    ...
    _glob = model.family.rsplit("/", 1)[1]  # documented; future filter use
    try:
...
def _evaluate_closed_source(candidate: _ClosedSourceCandidate, api) -> _OpenWeightsRow:
    try:
        ...
        is_not_found = "RepositoryNotFoundError" in type(exc).__name__ or "404" in str(exc)
        if is_not_found:
    ...
    weights = [f for f in files if any(fnmatch.fnmatch(f, pat) for pat in WEIGHT_GLOBS)]
    return _OpenWeightsRow(
        repo_id=candidate.id,
        note=candidate.note,
