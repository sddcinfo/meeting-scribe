...
CORPUS = Path(__file__).resolve().parents[2] / "benchmarks" / "corpus" / "en_ja_meeting_v2.jsonl"
...
_PUNCT_RE = re.compile(r"[、。「」『』！？!?,.\s]+")  # noqa: RUF001  # CJK fullwidth punctuation is intentional
    ...
    async with httpx.AsyncClient(timeout=120) as c:
        for i, (cid, ja_text) in enumerate(pairs, 1):
            if args.corpus_source == "tts_synth":
                ...
                wav_path = Path(args.fixture_dir) / "asr" / f"{cid}.wav"
