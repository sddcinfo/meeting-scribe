...
_PUNCT = re.compile(r"[、。「」『』！？!?,.\s]+")  # noqa: RUF001  # CJK fullwidth punctuation is intentional
...
def _detect_lang(text: str) -> str:
    ...
    if not text:
        return "?"
    ja = sum(1 for c in text if "぀" <= c <= "ヿ" or "一" <= c <= "鿿" or "ㇰ" <= c <= "ㇿ")
    if ja >= 1:
        return "ja"
    return "en"
...
async def _bench_one_meeting(
    ...
    (out_dir / f"{meeting_id}.json").write_text(
        json.dumps({"meeting_id": meeting_id, "rows": rows}, indent=2)
    )
...
def _summarize(rows: list[dict]) -> dict:
    ...
    out["latency_ms"] = {
        "qwen": {
            "p50": _percentile(
                [r["qwen"]["ms"] for r in speech if not r["qwen"].get("error")], 0.5
            ),
            "p95": _percentile(
                [r["qwen"]["ms"] for r in speech if not r["qwen"].get("error")], 0.95
            ),
            "n": sum(1 for r in speech if not r["qwen"].get("error")),
        },
        "cohere": {
            "p50": _percentile(
                [r["cohere"]["ms"] for r in speech if not r["cohere"].get("error")], 0.5
            ),
            "p95": _percentile(
                [r["cohere"]["ms"] for r in speech if not r["cohere"].get("error")], 0.95
            ),
