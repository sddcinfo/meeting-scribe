...
def _has_test_changes(diff: str) -> bool:
    ...
    for line in diff.split("\n"):
        if not line.startswith(("+++ b/", "--- a/")):
            continue
        path = line[6:]
        if ("/tests/" in path or path.startswith("tests/")) and (
            "test_" in path or path.endswith(".test.mjs") or path.endswith(".test.ts")
        ):
            return True
    return False
...
def _commit_trailer(commit: str = "HEAD") -> str | None:
    ...
    out = _run(
        ["git", "log", commit, "-n", "1", "--pretty=%(trailers:key=Bug-class,valueonly)"]
    ).stdout.strip()
    return out or None
...
def _fallback_heuristic(subjects: list[str], diff: str) -> dict:
    ...
    is_bugfix = any(s.startswith(("fix:", "fix(")) or "FIX " in s.upper() for s in subjects)
...
def main() -> int:
    ...
    if is_bugfix and not has_test:
        if _has_waiver_for_head():
            print(
                "⚠ pre-push: bugfix without test — waiver present, allowing push.", file=sys.stderr
            )
        else:
            head = _run(["git", "rev-parse", "HEAD"]).stdout.strip()
            ...
            print("  test under tests/ before pushing.", file=sys.stderr)
            print(file=sys.stderr)
            print("  To bypass (audit-trailed):", file=sys.stderr)
            print("    mkdir -p .git-waivers && \\", file=sys.stderr)
    ...
    if is_bugfix and not _commit_trailer():
        ...
        print("⚠ pre-push: bugfix commit is missing a `Bug-class:` trailer.", file=sys.stderr)
