...
def scan_file(path: Path) -> None:
    ...
    for i, line in enumerate(lines, 1):
        ...
        # ``DellMeetingAdmin<NNNN>`` is the deterministic admin
        # password for a v1.0 demo appliance — the literal IS the
        # device's admin credential. Any concrete 4-digit instance
        # checked into source identifies that device's password
        # forever (history rewrites are off-limits per the no-git-
        # reset rule). The pattern reference (``<NNNN>`` placeholder
        # or any non-4-digit form) is fine in docstrings + comments;
        # only the concrete instance is rejected.
        if re.search(r"DellMeetingAdmin[0-9]{4}\b", line):
            issues.append(f"{rel}:{i}: Concrete DellMeetingAdmin<NNNN> credential")
