...
def _probe(name: str, pin: dict) -> dict:
    ...
    try:
        ...
        if status != pin.get("expect_status", 200):
            ...
            return out
        expect_keys = pin.get("expect_keys", [])
        if expect_keys:
            try:
            ...
            if missing:
                ...
                return out
        # Empty expect_keys → status-only check. vLLM-style /health
        # returns 200 with empty body and no Content-Type, so don't
        # require a JSON-parseable payload when the pin isn't checking
        # any keys.
        out["ok"] = True
