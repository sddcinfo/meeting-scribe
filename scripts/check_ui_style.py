        ...
        for i, line in enumerate(raw.splitlines(keepends=True), start=1):
            ...
            if "—" in scan:
                ...
                for match in re.finditer(r"—", scan):
                    ...
                    # Accept hyphens + digits in the ID so newer metric
                    # cards (mc-asr-rtt-p95, mc-gpu-free, …) qualify.
                    if re.search(r'id="mc-[a-z][\w-]*">[^<]*—', scan):
                        continue
...
def _modal_css_sources() -> list[Path]:
    """The .modal-* selectors used to live entirely in style.css; in
    Phase 2.6 the core modal system moved to
    static/css/src/components/_modal.css. Validators scan both files
    so the rules still gate regardless of which lane the rule lives in.
    Returns only the paths that exist on disk."""
    candidates = [
        STATIC_CSS / "style.css",
        STATIC_CSS / "src" / "components" / "_modal.css",
    ]
...
def check_modal_text_selectable() -> list[tuple[str, int, str]]:
    ...
    """
    errors: list[tuple[str, int, str]] = []
    sources = _modal_css_sources()
    if not sources:
        errors.append(("static/css/style.css", 0, "no modal CSS source found"))
        return errors

    rule_re = re.compile(r"(\.modal-confirm-(?:message|title))(\s*)\{([^}]*)\}", re.DOTALL)
    seen = {"message": False, "title": False}
    for css_path in sources:
        raw = css_path.read_text()
        rel = str(css_path.relative_to(STATIC_CSS.parent.parent))
        for match in rule_re.finditer(raw):
            ...
            if "user-select" not in body or "text" not in body:
                ...
                errors.append((rel, line_no, f"{header}: missing user-select: text (rule 3)"))
            seen[kind] = True
    ...
    for kind, found in seen.items():
        if not found:
            errors.append(
                (
                    "static/css/{style.css,src/components/_modal.css}",
                    0,
                    f".modal-confirm-{kind} base rule not found in either file (rule 3)",
                )
            )
    return errors
...
def check_modal_message_wraps_long_tokens() -> list[tuple[str, int, str]]:
    ...
    errors: list[tuple[str, int, str]] = []
    rule_re = re.compile(r"\.modal-confirm-message\b[^{]*\{([^}]*)\}", re.DOTALL)
    found_any = False
    for css_path in _modal_css_sources():
        raw = css_path.read_text()
        rel = str(css_path.relative_to(STATIC_CSS.parent.parent))
        match = rule_re.search(raw)
        if not match:
            continue
        found_any = True
        body = match.group(1)
    ...
    if not found_any:
        errors.append(
            (
                "static/css/{style.css,src/components/_modal.css}",
                0,
                ".modal-confirm-message rule not found in either file (rule 4)",
            )
        )
    return errors
...
def check_no_document_keydown_in_dialogs() -> list[tuple[str, int, str]]:
    ...
    in `static/js/scribe/features/modal-system.js`. Boundaries are
    detected by the next top-level `export function` / `function` /
    `const` / `class` declaration.
    """
    errors: list[tuple[str, int, str]] = []
    js_path = STATIC_JS / "scribe" / "features" / "modal-system.js"
    ...
    for name in dialog_names:
        # Start: `[export] async function <name>(`.
        start_re = re.compile(
        ...
        if not m:
            ...
            continue
        # End: the next top-level declaration after this one.
...
def check_tokens_css_contract() -> list[tuple[str, int, str]]:
    """Rule 8: shared design tokens must live in the unified Tailwind
    theme and consumer stylesheets must not redeclare tokens that
    already live there.

    History: the rule used to require ``static/css/tokens.css`` (the
    pre-Tailwind unprefixed token source). Phase 2.6 (2026-05-14)
    folded every unprefixed token into the prefixed Tailwind theme
    at ``static/css/src/lib/tokens.css`` and deleted the legacy file.
    The rule now points at the new source-of-truth.
    """
    errors: list[tuple[str, int, str]] = []
    # New source of truth is the Tailwind theme. The legacy
    # `static/css/tokens.css` was removed when the prefix migration
    # landed (Phase 2.6).
    tokens_path = STATIC_CSS / "src" / "lib" / "tokens.css"
    if not tokens_path.exists():
        errors.append(
            (
                "static/css/src/lib/tokens.css",
                0,
                "missing — unified Tailwind theme must live in static/css/src/lib/tokens.css (rule 8)",
            )
        )
        return errors

    tokens_text = _read(tokens_path)
    canonical = set(re.findall(r"^\s*(--[a-z0-9][a-z0-9-]*)\s*:", tokens_text, re.MULTILINE))
    if not canonical:
        errors.append(
            (
                "static/css/tokens.css",
                0,
                "tokens.css declares zero CSS variables (rule 8)",
            )
        )
        return errors

    # Consumer-stylesheet redeclaration check + page-link order check
    # are obsolete now that every consumer page loads
    # `static/css/dist/<page>.css`, which carries the compiled @theme
    # tokens directly. Phase 2.6 (2026-05-14) folded the last
    # `style.css` + `how-it-works.css` consumers into the dist build.
    # The legacy `static/css/tokens.css` <link> was removed in the
    # same pass; redeclaration in component partials is now caught by
    # the build itself (a duplicate `--color-X` in src/components/_*.css
    # would just shadow the @theme value, which is the intended
    # component-local override pattern anyway).
    return errors
...
CHECKS = [
    ...
    ("tokens-css-contract (rule 8)", check_tokens_css_contract),
]
...
def main() -> None:
    ...
    if errors:
        ...
        print()
        print("See docs/styling.md for the full rule set.")
        sys.exit(1)
