#!/usr/bin/env bash
# meeting-scribe bootstrap — one-command setup for a fresh GB10 (Dell Pro Max with GB10).
#
...
if ! command -v apt-get >/dev/null 2>&1; then
    cat >&2 <<EOF
bootstrap.sh: apt-get not found. This script targets Debian/Ubuntu (the
Dell Pro Max with GB10 ships Dell DGX OS, an Ubuntu derivative). Other
distros aren't tested — the OS-package install in step 3 + the sudoers
rule paths in step 5 assume Debian-style /usr/bin/* paths.
EOF
    exit 1
fi

# Pre-flight banner so the operator knows what they're in for.
cat <<EOF

[bootstrap] meeting-scribe install — expected time + disk on first run:
[bootstrap]   ~30–50 minutes wall-clock, mostly model downloads
[bootstrap]   ~115 GB disk: vllm/vllm-openai image (~24 GB) + HF weights
[bootstrap]                 (~90 GB: ASR + TTS + diarization + Qwen3.6-FP8)
[bootstrap]   Idempotent — re-running on an installed box is fast.
[bootstrap]   Pass --yes (or set MEETING_SCRIBE_YES=1) to skip prompts.
...
disable_conflicting_dgx_oobe_services() {
    ...
    # The OOBE flow's leftover dnsmasq snippet conflicts with our
    # captive-portal config: it tells dnsmasq to bind 10.92.7.127
    # (the OOBE hotspot's IP, which doesn't exist on a meeting-scribe
    # appliance), causing NetworkManager's shared-mode dnsmasq to
    # fail with "Cannot assign requested address" on every AP up.
    # Bug surfaced 2026-05-05 — phone couldn't see DNS responses
    # because dnsmasq was crashing instantly. Removing the snippet
    # is safe; the OOBE services it served are masked above.
    if [[ -f /etc/NetworkManager/dnsmasq-shared.d/dgx-oobe.conf ]]; then
        rm -f /etc/NetworkManager/dnsmasq-shared.d/dgx-oobe.conf
        echo "[bootstrap] removed conflicting dgx-oobe dnsmasq snippet"
    fi

    # Stale netplan-NM-* YAML files. NetworkManager mirrors every
    # connection it manages into /etc/netplan/90-NM-<uuid>.yaml so
    # netplan can render them on next boot. After our wifi.py
    # delete-and-readds the AP profile (which it does on every
    # security-mode rotation), the OLD YAML stays behind. NM tries
    # to load all of them on restart and chokes on the
    # invalid-pmf-with-key-mgmt-none combinations from earlier
    # iterations, dropping into "supplicant-timeout" hell.
    # Bug surfaced 2026-05-05 — ``Dell Meeting Setup`` couldn't
    # come up after a previous OWE/SAE rotation because two
    # incompatible YAML files referenced the same SSID.
    local nm_yaml_pattern='/etc/netplan/90-NM-*.yaml'
    local removed_count=0
    for f in $nm_yaml_pattern; do
        [[ -f "${f}" ]] || continue
        if grep -qE 'Dell Meeting|DellDemo-AP' "${f}" 2>/dev/null; then
            rm -f "${f}"
            removed_count=$((removed_count + 1))
        fi
    done
    if (( removed_count > 0 )); then
        echo "[bootstrap] removed ${removed_count} stale netplan-NM-*.yaml AP entries"
...
need_pkgs=()
# ipset is the userspace driver for the captive-gateway per-IP allowlist
# (Phase H). The kernel ip_set / xt_set modules ship with the standard
# Ubuntu/Debian kernel; only the userspace tool needs apt installation.
for pkg in ffmpeg libportaudio2 libsndfile1 docker-compose-plugin rfkill iw ipset; do
...
${TARGET_USER} ALL=(root) NOPASSWD: /usr/bin/nmcli, /usr/sbin/iw, /usr/sbin/rfkill, /usr/sbin/iptables, /usr/sbin/ip6tables, /usr/sbin/setcap, /usr/bin/rm /etc/netplan/90-NM-*.yaml, /usr/sbin/netplan generate, /usr/bin/rm -f /var/lib/meeting-scribe/*, /usr/bin/rm -rf /var/lib/meeting-scribe/setup-pending
"
...
# ── 5.5 Kernel memory tuning (sysctl) ─────────────────────────────
# Ubuntu's default vm.swappiness=60 preemptively swaps out anonymous
# pages (the live Python process, vLLM working set) to keep page
# cache resident. On a meeting-scribe box that's backwards — the
# interactive web server's hot pages must stay resident or audio
# playback goes choppy and the UI lags. Drop swappiness to 10:
# swap stays available as a safety valve under real pressure but
# the kernel stops paging out interactive services to satisfy
# cache demand.
#
# Idempotent: re-running bootstrap.sh just re-copies the same
# content, sysctl --system reapplies cleanly.
SYSCTL_SRC="${SCRIPT_DIR}/provisioning/sysctl/60-meeting-scribe.conf"
SYSCTL_DST="/etc/sysctl.d/60-meeting-scribe.conf"
if [[ -f "${SYSCTL_SRC}" ]]; then
    install -m 0644 "${SYSCTL_SRC}" "${SYSCTL_DST}"
    if sysctl --system >/dev/null 2>&1; then
        CURRENT_SWAPPINESS="$(sysctl -n vm.swappiness 2>/dev/null || echo '?')"
        echo "[bootstrap] installed ${SYSCTL_DST} (vm.swappiness now: ${CURRENT_SWAPPINESS})"
    else
        echo "[bootstrap] WARNING: ${SYSCTL_DST} installed but sysctl --system failed" >&2
    fi
else
    echo "[bootstrap] WARNING: ${SYSCTL_SRC} missing; kernel tuning skipped" >&2
fi
...
# Self-heal: recover from wrapper-truncation incidents (2026-05-07, 2026-05-12)
# and python-version drift between mise.toml and the existing .venv. Idempotent —
# exits 0 with no changes when healthy, rebuilds in place otherwise. Must run
# AFTER the editable install above so the lockfile path is consulted on rebuild.
echo "[bootstrap] === venv self-heal (truncated wrappers / python drift) ==="
as_user 'scripts/repair_venv.py'

# ── 7. Sister-clone auto-sre ──────────────────────────────────────
# auto-sre is cloned as a sibling directory so the translate vLLM
# container can be managed independently of meeting-scribe's lifecycle.
# The relative path is fixed at ../auto-sre — if you cloned this repo
# to a non-standard directory name, the path resolves from THIS repo's
# parent dir.
if [[ "${MEETING_SCRIBE_SKIP_AUTOSRE:-0}" != "1" ]]; then
    ...
    "; then
        echo "[bootstrap] FATAL: auto-sre clone or install failed." >&2
        echo "[bootstrap]   Resolve the error above and re-run, OR pass" >&2
        echo "[bootstrap]   MEETING_SCRIBE_SKIP_AUTOSRE=1 to defer the autosre install." >&2
        echo "[bootstrap]   (Translation backend won't work without autosre.)" >&2
        exit 1
    fi
    ...
    echo "[bootstrap]   note: translate vLLM will be unavailable until autosre is installed."
fi
...
# Prefer /data/huggingface (large mounted volume on production GB10s)
# when the parent /data exists. Otherwise fall back to the per-user
# default at ~/.cache/huggingface so a laptop/dev box without /data
# isn't required to create one.
echo
if [[ -d /data && ! -d /data/huggingface ]]; then
    install -d -o "${TARGET_USER}" -g "$(id -gn "${TARGET_USER}")" -m 0775 /data/huggingface
    echo "[bootstrap] HF cache target: /data/huggingface"
elif [[ -d /data/huggingface ]]; then
    echo "[bootstrap] HF cache target: /data/huggingface (existing)"
else
    echo "[bootstrap] HF cache target: ${TARGET_HOME}/.cache/huggingface (default; no /data mount)"
    install -d -o "${TARGET_USER}" -g "$(id -gn "${TARGET_USER}")" -m 0775 \
...
install_helper_systemd_unit() {
    ...
    # Make ``meeting-scribe`` callable from any shell — no venv
    # activation, no PATH gymnastics, no shell-local var dance. The
    # binary lives under the user's home (per-user editable install)
    # but the wrapper at /usr/local/bin/ resolves it via the
    # /usr/local/lib/meeting-scribe symlink we just installed, so
    # root + delldemo + any future SSH session all see the same
    # entrypoint. Idempotent: ln -sf overwrites a stale link.
    ln -sf "${installed_lib}/.venv/bin/meeting-scribe" /usr/local/bin/meeting-scribe
...
# Phase C: per-device mDNS publishing + hostnamectl rename.
install_mdns_systemd_unit() {
    local mdns_unit_src="${SCRIPT_DIR}/provisioning/systemd/meeting-scribe-mdns.service"
    local mdns_unit_dst=/etc/systemd/system/meeting-scribe-mdns.service

    if [[ ! -f "${mdns_unit_src}" ]]; then
        echo "[bootstrap] mdns unit source missing: ${mdns_unit_src}" >&2
        return 1
    fi

    # Avahi packages — apt-installable on the customer GB10 image.
    if ! command -v avahi-publish >/dev/null 2>&1; then
        echo "[bootstrap] installing avahi-daemon avahi-utils"
        DEBIAN_FRONTEND=noninteractive apt-get install -y avahi-daemon avahi-utils \
            >/dev/null 2>&1 || \
            echo "[bootstrap] WARN: avahi install failed — mDNS service will not start" >&2
    fi

    # Per-device hostname so multi-appliance LANs don't collide on
    # ``meeting-scribe-${id4}.local``. Reads the appliance ID from
    # the same file ``cli/_common.py`` consults.
    local aid_file=/etc/meeting-scribe/appliance-id
    if [[ ! -r "${aid_file}" ]]; then
        aid_file="${SCRIPT_DIR}/certs/appliance-id"
    fi
    if [[ -r "${aid_file}" ]]; then
        local id4
        id4="$(head -c 4 "${aid_file}" 2>/dev/null || true)"
        if [[ -n "${id4}" ]]; then
            hostnamectl set-hostname "meeting-scribe-${id4}" 2>&1 | tail -3 || true
        fi
    fi

    install -m 0644 "${mdns_unit_src}" "${mdns_unit_dst}"
    systemctl daemon-reload
    systemctl enable --now avahi-daemon.service 2>&1 | tail -3 || true
    systemctl enable --now meeting-scribe-mdns.service 2>&1 | tail -3 || \
        echo "[bootstrap] WARN: meeting-scribe-mdns.service start failed" >&2
}

install_mdns_systemd_unit || \
    echo "[bootstrap] mdns install reported issues — see above"

# ── 12.5 Speakerphone udev rule (USB HID-telephony devices) ───────
# Grants the unprivileged service user (member of ``plugdev``) rw
# access on the SP325/SP3022 event* and hidraw* nodes so the
# ``meeting-scribe speakerphone listen`` daemon can read button
# events and drive the Mute LED ring. The rule is shipped at
# ``provisioning/udev/99-sddc-speakerphone.rules`` and copied into
# ``/etc/udev/rules.d/`` here so a fresh boot wires it up before any
# console session is needed. No-op on systems without ``udevadm``.
install_speakerphone_udev_rule() {
    local src="${SCRIPT_DIR}/provisioning/udev/99-sddc-speakerphone.rules"
    local dst="/etc/udev/rules.d/99-sddc-speakerphone.rules"
    if [[ ! -f "${src}" ]]; then
        echo "[bootstrap] speakerphone udev rule source missing: ${src}" >&2
        return 0
    fi
    install -m 0644 "${src}" "${dst}"
    if command -v udevadm >/dev/null 2>&1; then
        udevadm control --reload 2>&1 | tail -3 || true
        udevadm trigger 2>&1 | tail -3 || true
    fi
    echo "[bootstrap] speakerphone udev rule installed at ${dst}"
}

install_speakerphone_udev_rule || \
    echo "[bootstrap] speakerphone udev install reported issues — see above"

# ── 13. Install user systemd unit (boot autostart) ────────────────
# Linger MUST be enabled BEFORE we install the user unit. Without
# linger, the user-systemd manager only lives for the duration of an
# active login session — which means the moment the operator's SSH
# disconnects, every user service (including meeting-scribe) gets
# torn down. This bug surfaced 2026-05-06 cold-wipe testing: the
# docstring at the top of this file claims linger is set, but the
# actual ``loginctl enable-linger`` call was missing.
if ! loginctl show-user "${TARGET_USER}" --property=Linger 2>/dev/null | grep -q "Linger=yes"; then
    loginctl enable-linger "${TARGET_USER}" \
        && echo "[bootstrap] linger enabled for ${TARGET_USER} (user services survive logout)" \
        || echo "[bootstrap] WARN: loginctl enable-linger ${TARGET_USER} failed" >&2
fi

echo
echo "[bootstrap] installing meeting-scribe.service (user systemd unit)"
# Invoke via ``python3 -m meeting_scribe`` (not ``meeting-scribe``) so a
# truncated console-script wrapper from a freshly-completed
# ``pip install -e .`` cannot block install-service from rendering the
# unit. See docs/known-issues/wrapper-truncation.md.
as_user '.venv/bin/python3 -m meeting_scribe install-service --no-start' || \
    echo "[bootstrap] install-service reported issues — see above"

# ── 13.5 Install the speakerphone listener unit ───────────────────
# Same Python-via-module trick as the main install-service call —
# the console-script wrapper may be truncated until after the start
# in step 14, but ``python3 -m meeting_scribe speakerphone install``
# only renders the unit file and does not need the wrapper.
echo
echo "[bootstrap] installing meeting-scribe-speakerphone.service (user systemd unit)"
as_user '.venv/bin/python3 -m meeting_scribe speakerphone install --no-start' || \
    echo "[bootstrap] speakerphone install reported issues — see above"
