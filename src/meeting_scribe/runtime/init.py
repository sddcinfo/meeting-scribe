...
def _tts_restart_containers() -> tuple[str, ...]:
    """Containers to restart when TTS marks itself degraded.

    GB10 reliability mode owns one resident compose service. If an operator
    points ``SCRIBE_TTS_VLLM_URL`` at an experimental external pool, we still
    only restart the managed local service unless they explicitly override
    ``SCRIBE_TTS_RESTART_CONTAINERS``.
    """
    raw = os.environ.get("SCRIBE_TTS_RESTART_CONTAINERS")
    if raw:
        return tuple(name.strip() for name in raw.split(",") if name.strip())

    urls = []
    if state.tts_backend is not None:
        urls = list(getattr(state.tts_backend, "_urls", []) or [])
    if not urls:
        configured = state.config.tts_vllm_url or ""
    ...
    while True:
        ...
        try:
            ...
            if state.tts_backend is None:
                ...
                # TTS containers can report /health=200 while synthesis
                # returns CUDA 500s. Treat degraded as requiring container
                # restarts; shallow health is not a recovery signal here.
                containers = _tts_restart_containers()
                logger.warning("TTS degraded; restarting TTS containers: %s", ", ".join(containers))
                restarted = False
                for container_name in containers:
                    restarted = await _restart_container(container_name) or restarted
                if restarted:
                    reset = getattr(state.tts_backend, "reset_failure_state_after_restart", None)
                    if reset is not None:
                        await reset()
                    logger.info("TTS failure state reset after container restart")
