...
async def _eager_summary_loop(meeting_id: str) -> None:
    ...
    Polls every 60 seconds after a 60-second warmup, but only fires a
    draft when the transcript has both ≥ 20 final events and has grown
    ≥ 30 % since the last draft.  The 20-event floor lets short
    meetings (≈ 3 min of speech) get at least one draft so the
    stop-time draft-acceptance gate can elide the slow regen call on
    Qwen3.6-FP8.  Uses vLLM priority -5 (just above translation's -10)
    so real-time translation still preempts but the draft has a
    fighting chance to keep up with longer meetings.  The cached draft
    is used at stop time if the transcript hasn't grown significantly
    (> 20 % growth), avoiding the full LLM call during finalization.
    ...
    Tuning history (A2 step 3, finalised after telemetry landed):
      ...
      * Priority 5 → -5 — translation still preempts (it's at -10);
        but TTS / furigana no longer block draft generation entirely
        under contention.
    """
    ...
    while state.current_meeting and state.current_meeting.meeting_id == meeting_id:
        try:
            ...
            # Skip if too few events (< 20) or transcript hasn't grown
            # significantly since last draft (< 30 % growth). 20-event
            # threshold lets short meetings (~3 min of speech) get at
            # least one draft so the stop-time draft-acceptance gate
            # can elide the slow regen on Qwen3.6-FP8.
            if n_finals < 20 or n_finals == state._eager_summary_event_count:
        ...
        # 60 s between drafts. The 30 %-growth + 20-event gates above
        # already prevent spamming the GPU; tightening from 5 min to 1
        # min lets the cached draft stay close to the live transcript
        # so the stop-time growth check has a chance to accept it on
        # short meetings (which previously only saw one eager run).
        await asyncio.sleep(60)
        ...
        while True:
            ...
            for segment_id, event in list(state._pending_speaker_events.items()):
                ...
                try:
                    ...
                    pass  # broadcast is opportunistic — journal already has the new revision

                if (
                    os.environ.get("SCRIBE_TTS_DEFER_UNTIL_CATCH_UP", "0") == "1"
                    and updated.translation
                    and updated.translation.status.value == "done"
                    and updated.translation.text
                    and state.tts_backend
                    and state.tts_backend.available
                ):
                    try:
                        from meeting_scribe.tts.worker import _enqueue_tts

                        speaker = updated.speakers[0] if updated.speakers else None
                        if speaker is None:
                            speaker_id = "default"
                        ...
                        if any(
                            getattr(pref, "delivery_mode", "simultaneous") != "drop"
                            for pref in state._audio_out_prefs.values()
                        ):
                            logger.info(
                                "TTS fire after speaker catch-up: seg=%s speaker=%s text=%r",
                                updated.segment_id,
                                speaker_id,
                                (updated.translation.text or "")[:60],
                            )
                            _enqueue_tts(updated, speaker_id)
                    except Exception:
                        logger.exception("speaker catch-up TTS enqueue failed")
