...
* ``silence_watchdog_loop`` — emits a ``meeting_warning`` WS broadcast
  ...
  ``state.last_nonzero_audio_ts`` (mutated by ``ws.audio_input`` when
  a frame's peak crosses the ``_AUDIO_LIVENESS_FLOOR``) so the alarm
  also catches the 2026-05-14 failure mode: frames arriving but all
  samples zero (USB stale routing, hardware mute, dead ADC).
...
SILENCE_WATCHDOG_POLL_INTERVAL_S: float = 1.0


async def silence_watchdog_loop() -> None:
    """Emit a ``meeting_warning`` WS broadcast when audio is silent or stalled.

    Two failure modes the watchdog must catch:
    1. **Frames stopped arriving** — browser WS dropped, NAT timed out,
       CPU starved. The old failure mode the watchdog was built for.
    2. **Frames arriving but all-zero** — the failure that hid the
       2026-05-14 demo bug. A USB mic with a stale routing binding, a
       hardware-muted SP325, or a kernel-side ALSA hang can all leave
       the capture process happy while the data is silent.

    Both modes are unified by reading ``state.last_nonzero_audio_ts``,
    which only advances when a frame's peak exceeds ``_AUDIO_LIVENESS_FLOOR``
    (= 1e-4, well below speech). Stale by 5+ s = something is wrong.

    Module-level ``SILENCE_WATCHDOG_POLL_INTERVAL_S`` so the unit
    test suite can shrink it to milliseconds without spending 2 s
    of wall-clock per test waiting for two real polls.
    """
    while True:
        await asyncio.sleep(SILENCE_WATCHDOG_POLL_INTERVAL_S)
        try:
            if not state.current_meeting:
                state.silence_warn_sent = False
                continue
            # Anchor against the current meeting's start so we only count
            # silence accumulated WITHIN this meeting — prevents the
            # watchdog from firing instantly with "Hs since last non-zero
            # frame from a prior meeting" the moment a new one starts.
            meeting_start = state.metrics.meeting_start
            if not meeting_start:
                state.silence_warn_sent = False
                continue
            anchor = max(state.last_nonzero_audio_ts, meeting_start)
            age = time.monotonic() - anchor
            if age >= _SILENCE_WARN_THRESHOLD_S and not state.silence_warn_sent:
                logger.error(
                    "Silence watchdog: no non-zero audio in %.0fs for meeting %s — "
                    "frames may be arriving but samples are all-zero (check mic device, "
                    "USB cable, hardware mute, gain) — notifying clients",
                    age,
