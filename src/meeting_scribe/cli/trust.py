...
def _platform_uninstall(*, appliance_id: str | None, all_meeting_scribe: bool) -> tuple[bool, str]:
    ...
    if system == "Windows":
        return _uninstall_windows(appliance_id=appliance_id, all_meeting_scribe=all_meeting_scribe)
    return False, f"unsupported platform: {system}"
...
def _uninstall_macos(*, appliance_id: str | None, all_meeting_scribe: bool) -> tuple[bool, str]:
    if shutil.which("security") is None:
        return False, "macOS `security` binary not on PATH"
...
def _uninstall_linux_nss(*, appliance_id: str | None, all_meeting_scribe: bool) -> tuple[bool, str]:
    if shutil.which("certutil") is None:
...
def _uninstall_windows(*, appliance_id: str | None, all_meeting_scribe: bool) -> tuple[bool, str]:
...
def _fetch_leaf_pem(host: str, port: int = 443) -> str:
    ...
    # Pin to TLS 1.2+ even though we don't verify the leaf — the cert
    # we're fetching is operator-known, but a downgrade attacker on
    # the LAN could still force TLS 1.0/1.1 negotiation. The peer
    # cert lookup itself doesn't care, so the only thing TLS 1.2 buys
    # us here is shutting up the CodeQL "weak TLS" rule; cost is zero.
    ctx.minimum_version = ssl.TLSVersion.TLSv1_2
    import socket

    with (
        socket.create_connection((host, port), timeout=10.0) as raw,
        ctx.wrap_socket(raw, server_hostname=host) as ssock,
    ):
