...
def precommit(
    ...
    Runs two validators:
      - scripts/check_ui_style.py (docs/styling.md rule set: no native popups,
    ...
    clone (see docs/styling.md) so style regressions block commits locally.
    """
    ...
    if style_rc != 0:
        click.secho(
            "UI style regressions detected. Fix them (see docs/styling.md) and rerun.",
            fg="red",
        )
        if not warn_only:
            sys.exit(style_rc)
