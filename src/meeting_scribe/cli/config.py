...
slide_use_json_schema, debug_audio_timing) persist to
``runtime-config.json`` and SIGHUP the running server so it picks them
...
def config_group() -> None:
    ...
    \b
    These settings can be flipped without restarting the server:
      translate_url          — live-translation vLLM endpoint
      slide_translate_url    — slide-pipeline vLLM endpoint
      slide_use_json_schema  — response_format flag for slide JSON
      debug_audio_timing     — log per-request /api/meetings/.../audio
                                timing (off by default; flip on to
                                diagnose playback latency reports)
...
def config_set(key: str, value: str, no_reload: bool) -> None:
    ...
    _BOOL_KEYS = {"slide_use_json_schema", "debug_audio_timing"}
    if key in _BOOL_KEYS:
        if value.lower() in {"true", "1", "yes", "on"}:
            ...
            raise click.BadParameter(f"{key} expects a boolean (true/false), got {value!r}")
