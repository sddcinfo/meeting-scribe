...
def demo_smoke(
    ...
    Sequence:
      0. ``preflight --mode=demo`` (Phase 6 hardware gate): mic
         capturing, SP325 wideband compliance, AP profile valid,
         hotspot up. Fails fast before the E2E flow so a broken
         demo never gets past the first 2 seconds.
    ...
    # Phase 6 gate. Skipping hardware is a deliberate CI-only thing —
    # operators on the GB10 always get the full check set so an actual
    # demo gets the actual readiness signal.
    click.echo("[demo-smoke] 0/4 running preflight --mode=demo…")
    from meeting_scribe import preflight as _pf

    gate_code = _pf.cmd_demo(wait_seconds=float(total_budget), skip_hardware=False)
    if gate_code != _pf.EXIT_OK:
        click.secho(
            f"[demo-smoke] preflight --mode=demo failed (exit {gate_code}) — "
            "fix the FAIL lines above before re-running",
            fg="red",
            err=True,
        )
        sys.exit(gate_code)
