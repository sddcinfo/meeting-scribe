...
def _render_unit() -> str:
    venv_bin = _venv_bin()
    # Every Exec* line invokes the venv interpreter directly via
    # ``python3 -m meeting_scribe`` instead of the
    # ``.venv/bin/meeting-scribe`` console-script wrapper. The wrapper
    # has been observed to land as a bare ``#!/bin/sh\n`` after
    # ``pip install -e .`` races; when that happens, systemd's
    # ``Type=notify`` start fails with ``Result: protocol`` because the
    # truncated shell script exits 0 without ever sending ``READY=1``.
    # ``-m meeting_scribe`` (resolved by ``__main__.py``) invokes the
    # CLI through Python directly, sidestepping the wrapper entirely.
    # See docs/known-issues/wrapper-truncation.md for the residual-risk
    # tracking.
    py = f"{venv_bin}/python3 -m meeting_scribe"
    return f"""[Unit]
...
# Phase 0 — cheap, filesystem-only hard gates. Exit 64/65 are reserved
# for non-retriable failures and are prevented from triggering
# Restart=on-failure via RestartPreventExitStatus below.
ExecCondition={py} preflight --mode=precondition

# Phase 0.5 — guarantee the scribe compose stack exists before any
# health check runs. ``gb10 up --offline`` is idempotent; cheap on
# the happy path and load-bearing on a disaster path where a prior
# ``docker compose down`` left the containers destroyed.
ExecStartPre={py} gb10 up --offline

# Phase 1 + Phase 2 — Docker remediation and live backend readiness.
# 720s wait covers cold-boot autosre-vllm-local (~7 min for 35B
# weights). Single shared budget for all probes; each clamps per-call
# timeout to min(remaining, per-check cap).
ExecStartPre={py} preflight --mode=boot --wait 720

# ``--foreground`` skips the daemonize fork — systemd needs the
# top-level PID to receive READY=1 over NotifyAccess=main.
ExecStart={py} start --foreground

# Static audit on every stop. Never blocks shutdown (exit 0 always)
# but writes BOOT_BLOCKED if static checks fail, so a subsequent
# cold boot lands in condition=failed with a clear reason.
ExecStopPost={py} preflight --mode=shutdown

KillMode=control-group
KillSignal=SIGTERM
# Worst-case cold path: up to 720s preflight + ~240s backend warmup +
# headroom = 1200s. If you hit this, something is legitimately wrong —
