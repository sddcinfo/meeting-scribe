...
def factory_reset_cmd(yes: bool) -> None:
    """Wipe setup state + admin credentials so the box re-enters first-touch.

    Atomically:
      * bumps ``auth-version`` (every existing admin cookie fails verify)
      * removes ``setup-complete``, ``setup-pending/``, ``setup-committing/``
      * removes the LIVE HMACs (admin password + recovery code)
      * clears the AP password from the persisted settings store

    A subsequent ``meeting-scribe restart`` lands the appliance in
    setup-mode (open ``Dell Meeting Setup`` AP, wizard at
    ``https://meeting-scribe-${id4}.local/setup``).
    """
    from meeting_scribe import setup_state

    if not yes:
        click.confirm(
            "This will wipe the admin password, recovery code, and AP credentials. Continue?",
            abort=True,
        )

    # No rotating-secret artifact to regenerate beyond what
    # setup_state.factory_reset already handles internally.
    regenerate_secrets = None

    # Also rotate the AP password persisted in settings_store so a
    # leaked locked-mode PSK can't survive a factory reset.
    try:
        ...
        click.secho(f"warning: could not clear settings_store: {exc}", fg="yellow")

    setup_state.factory_reset(regenerate_secrets=regenerate_secrets)
    click.secho("[OK] factory reset complete", fg="green")
    click.echo("Run `meeting-scribe restart` to enter setup mode.")
