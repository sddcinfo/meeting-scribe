...
async def _deliver_audio_to_listener(
    ...
    if fmt == "wav-pcm":
        ...
        if wav_bytes is None:
            # Run the numpy-heavy WAV build in a thread so a long TTS
            # output (≥1 s of audio at 24 kHz) doesn't block the asyncio
            # loop while ASR / translation are alive on the same loop.
            # The cache means this is at most ONCE per segment per rate
            # regardless of listener count.
            wav_bytes = await asyncio.to_thread(_build_riff_wav, audio, source_rate)
            if wav_cache is not None:
...
async def _send_passthrough_audio(audio: np.ndarray, source_language: str) -> None:
    ...
    recipients = [
        ...
        and state._audio_out_prefs[ws].delivery_mode != "drop"
        and state._audio_out_prefs[ws].delivery_mode != "consecutive"
...
async def _send_audio_to_listeners(
    ...
    for ws in list(state._audio_out_clients):
        ...
        if client_voice != voice_mode:
            continue
        if pref.delivery_mode == "consecutive":
            continue
    ...
    for ws in dead:
        ...
        state._audio_out_prefs.pop(ws, None)
        state.metrics.listener_removed_on_send_error += 1
    if event is not None and state.interpretation_buffer is not None:
        ...
        for ws in state._audio_out_clients:
            ...
            if pref_lang and pref_lang != target_norm:
                continue
            if getattr(pref, "voice_mode", "studio") != voice_mode:
                continue
            has_consecutive = True
            break
        if has_consecutive:
            await state.interpretation_buffer.append(
                event=event,
                audio=audio,
                source_lang=getattr(event, "language", ""),
