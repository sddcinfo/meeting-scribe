...
class Resampler:
    ...
    def _get_resampler(self, source_rate: int, target_rate: int) -> torchaudio.transforms.Resample:
        ...
        Uses Kaiser-windowed sinc interpolation with parameters tuned for
        "best quality" per the torchaudio resampling tutorial:
            lowpass_filter_width=16  (wider filter → sharper rolloff)
            rolloff=0.946  (more conservative cutoff to avoid aliasing)
            resampling_method="sinc_interp_kaiser"
            beta=14.769656459379492  (Kaiser window beta for ~-80dB stopband)

        Reference: https://docs.pytorch.org/audio/stable/tutorials/audio_resampling_tutorial.html
        """
        ...
        if cache_key not in self._resamplers:
            ...
            logger.info(
                "Created resampler: %d Hz -> %d Hz (Kaiser sinc, best quality)",
                source_rate,
                target_rate,
            )
