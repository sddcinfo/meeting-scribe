...
def _run_extract(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        # Extract speaker notes alongside the body text. Notes feed
        # the admin "translation + presentation" view (a scrollable
        # panel below the slide thumbs) so the operator sees the
        # talking points prepared for each slide. Notes failures are
        # never fatal — the deck still renders without them.
        try:
            import json as _json

            notes_list = extract_notes_from_pptx(input_path)
            (output_dir / "slide_notes.json").write_text(
                _json.dumps({"notes": notes_list}, ensure_ascii=False, indent=2)
            )
