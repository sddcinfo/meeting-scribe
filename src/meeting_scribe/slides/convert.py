...
def extract_notes_from_pptx(pptx_path: Path) -> list[str]:
    """Extract speaker notes per slide as plain text.

    Returns ``len(prs.slides)`` strings — empty string for slides with
    no notes. Used by the admin "translation + presentation" view so
    the operator can see the slide's prepared talking points alongside
    the rendered thumbnail.

    python-pptx exposes notes via ``slide.notes_slide.notes_text_frame``;
    we walk paragraphs (preserving newlines) and join. PowerPoint
    auto-creates an empty notes slide on every slide, so a slide with
    no notes still has a notes_slide — we strip the result + drop the
    auto-inserted slide-number placeholder text python-pptx sometimes
    surfaces.
    """
    from pptx import Presentation
    ...
    for slide in prs.slides:
        ...
        try:
            ...
            logger.debug("notes extract failed for slide", exc_info=True)
            text = ""
        out.append(text)
    return out
...
def render_pptx_to_pdf(pptx_path: Path, dest_pdf: Path) -> None:
    """Render a PPTX into a PDF at ``dest_pdf`` (atomic).

    A thin public wrapper around ``_libreoffice_pptx_to_pdf`` for callers
    that want the deck-level PDF without re-running the full per-slide PNG
    render. Used by the post-express bulk render to produce the translated
    deck PDF without invalidating the per-slide PNGs the express batches
    already wrote.
    """
    dest_pdf.parent.mkdir(parents=True, exist_ok=True)
    tmp_dir = dest_pdf.parent / f"_pdf_tmp_{uuid.uuid4().hex[:8]}"
    tmp_dir.mkdir(parents=True, exist_ok=True)
    try:
        produced = _libreoffice_pptx_to_pdf(pptx_path, tmp_dir)
