...
from meeting_scribe.slides.worker import (
    run_partial_translated_render,
    run_reinsert,
    run_translated_pdf_only,
)
from meeting_scribe.util.atomic_io import atomic_write_json

logger = logging.getLogger(__name__)

# Per-deck slide-translation stats written as JSONL.  Consumed by
# A/B analysis scripts to compare parse-failure rates across
# translation model + response-format variants.  Path is user-level
# rather than repo-local so stats survive container rebuilds of the
# scribe service.  Eval runs override via the ``slide_stats_dir``
# runtime-config knob so artifacts land under the privacy-gated
# shadow root instead of leaking into the production path.
...
def _emit_deck_stats(*, meeting_id: str, deck_id: str, stats: _DeckStats) -> None:
    ...
    The schema here is the contract A/B analysis scripts read, so
...
class SlideJobRunner:
    ...
    async def _run_pipeline(
        ...
        try:
            ...
            if not missing:
                ...
                # Post-express PDF finalizer. The express path produces
                # per-slide PNGs but never emits the deck-level
                # translated.pptx / translated/original.pdf the downloads
                # UI advertises. Fire it in the background so it doesn't
                # delay the "complete" handoff to viewers — a follow-up
                # LO render (~5-15s on a 50-slide deck) just makes the
                # download row light up a beat later.
                asyncio.create_task(
                    self._finalize_translated_pdf(deck_id, deck_dir, pptx_bytes, all_translations),
                    name=f"trans-pdf-{deck_id[:8]}",
                )
                return
            ...
            if meta_path.exists():
                try:
                    ...
                    pass  # meta corrupt or unwritable — original failure already logged above

        finally:
            # Flush per-deck stats regardless of completion / cancel /
            # failure so drain + analysis always see an exit row.
            # Pull slides_total from the meta if available.
    ...
    async def _finalize_translated_pdf(
        self,
        deck_id: str,
        deck_dir: Path,
        pptx_bytes: bytes,
        ...
        """Post-express finalizer: emit translated.pptx + translated/original.pdf.

        Only the bulk-fallback path produces these deck-level artifacts as
        part of the main pipeline; the express path that handles most
        decks writes per-slide PNGs and exits. The downloads UI advertises
        a "Translated deck (PDF)" entry though, so we need the deck PDF
        to actually be on disk for every translated deck. Runs as a
        background task after the user-facing "complete" broadcast so the
        ~5-15s LO render doesn't gate slide visibility.
        """
        translated_pdf_path = deck_dir / "translated" / "original.pdf"
        if translated_pdf_path.exists():
            return
        if not translations:
            return
        try:
            await run_translated_pdf_only(pptx_bytes, translations, deck_dir)
            logger.info(
                "Slide job %s post-express PDF finalizer wrote translated.pdf",
                deck_id,
            )
        except Exception:
            # Don't surface this as a job failure — the slide pipeline
            # already returned success and the express PNGs are on disk.
            # Worst case the download row stays disabled with the 404 hint.
            logger.exception("Slide job %s post-express PDF finalizer failed", deck_id)
    ...
    def get_translated_pdf_path(self, meeting_id: str) -> Path | None:
        """Path to the lossless PDF render of the deck with translated
        text reinserted. Lives at ``{deck_dir}/translated/original.pdf``
        because ``convert_pptx_to_images`` writes a literal ``original.pdf``
        regardless of input — the path is named after the LO output, not
        the source.

        Absent for monolingual decks, and for express-only decks whose
        post-express bulk render hasn't completed yet.
        """
        deck_id = self.get_meeting_deck_id(meeting_id)
        if not deck_id:
            return None
        deck_dir = self._deck_dir(meeting_id, deck_id)
        path = deck_dir / "translated" / "original.pdf"
        if path.exists():
            return path
        return None
