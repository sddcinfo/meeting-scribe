...
def _extract_text_sync(output_dir: Path) -> dict:
    ...
    try:
        ...
        (output_dir / "slide_notes.json").write_text(
            json.dumps({"notes": notes_list}, ensure_ascii=False, indent=2)
        )
...
def _run_translated_pdf_only_sync(
    pptx_bytes: bytes,
    translations: list[dict],
    output_dir: Path,
) -> None:
    """Produce ``translated.pptx`` + ``translated/original.pdf`` only.

    Used as a post-express finalizer: the express batches already wrote
    per-slide PNGs, so we skip the PNG re-render and just emit the
    deck-level artifacts the downloads UI advertises. Both writes go
    through atomic-replace.
    """
    input_path = output_dir / f"_post_express_upload_{uuid.uuid4().hex[:8]}.pptx"
    input_path.write_bytes(pptx_bytes)
    try:
        tmp_pptx = output_dir / f"translated.pptx.tmp.{uuid.uuid4().hex[:8]}"
        reinsert_translated_text(input_path, translations, tmp_pptx)
        final_pptx = output_dir / "translated.pptx"
        tmp_pptx.replace(final_pptx)

        translated_dir = output_dir / "translated"
        translated_dir.mkdir(exist_ok=True)
        render_pptx_to_pdf(final_pptx, translated_dir / "original.pdf")
    finally:
        input_path.unlink(missing_ok=True)


async def run_translated_pdf_only(
    pptx_bytes: bytes,
    translations: list[dict],
    output_dir: Path,
) -> None:
    """Async wrapper for the post-express PDF finalizer."""
    await asyncio.to_thread(_run_translated_pdf_only_sync, pptx_bytes, translations, output_dir)
