...
def _is_suspected_room_self_echo(event: TranscriptEvent) -> bool:
    if not event.is_final or not event.text:
        return False
    event_lang = _norm_lang(event.language)
    for listener in state._audio_out_clients:
        pref = state._audio_out_prefs.get(listener)
        if pref is None or pref.transport != "room_sink":
            continue
        if event_lang != _norm_lang(getattr(listener, "last_played_target_lang", None)):
            continue
        playback_until = getattr(listener, "playback_until", 0.0)
        if event.utterance_end_at is None or event.utterance_end_at >= playback_until:
            continue
        played = getattr(listener, "last_played_text", "") or ""
        if _jaccard(event.text, played) >= 0.6:
            return True
    return False
...
async def _process_event(event: TranscriptEvent) -> None:
    ...
    if state.current_meeting is None:
        if event.is_final and event.text:
            logger.info(
                "ASR final dropped with no active meeting: seg=%s text=%r",
                event.segment_id,
                event.text[:40],
            )
        return
    ...
    if _is_suspected_room_self_echo(event):
        event = event.model_copy(update={"suspected_self_echo": True})
    ...
    if event.is_final and state.translation_queue and not event.suspected_self_echo:
...
async def _broadcast_translation(event: TranscriptEvent) -> None:
    ...
    if (
        ...
        try:
            ...
            logger.exception("TTS local-sink self-heal failed")

        # Skip synthesis entirely if no active listeners — saves GPU
        ...
        if active_listener_count == 0:
            logger.info(
                "TTS skip seg=%s: no active audio listeners (registered=%d)",
                event.segment_id,
                len(state._audio_out_prefs),
            )
            return
        target_lang = _norm_lang(event.translation.target_language)
        voice_modes_needed = _listener_tts_demand(target_lang)
        if not voice_modes_needed:
            logger.info(
                "TTS skip seg=%s: no matching listeners before enqueue "
                "(target_lang=%s, registered=%d)",
                event.segment_id,
                target_lang,
                len(state._audio_out_prefs),
            )
