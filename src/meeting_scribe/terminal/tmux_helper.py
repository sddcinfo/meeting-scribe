...
# Note on `window-size manual`: tested 2026-05-08 against tmux 3.4 and
# rejected. Combining it with `new-session -A -d` from a `-f` config
# crashes the tmux server during first-session spawn (verified via
# `tmux -vv`; the truncated `spawn_window` log shows the issue). Per-tab
# tmux sessions alone (one client per session) eliminate the multi-client
# clipping that motivated the option — `latest` mode trivially equals
# the lone client's size. Resize_window stays as a belt-and-suspenders
# call from pty_session.resize.
...
async def resize_window(
    ...
    *,
    ...
    """Force-resize a tmux window to ``cols x rows``.

    With ``window-size manual`` in scribe.tmux.conf, tmux's window size
    only changes when this command runs — TIOCSWINSZ on a client's PTY
    delivers SIGWINCH but tmux declines to reflow. Calling this AFTER
    the ioctl is what makes per-tab resize deterministic regardless of
    how many other clients are attached or what their sizes are.

    Returns True on success, False if the session doesn't exist or the
    tmux call failed/timed out.
    """
    if shutil.which("tmux") is None:
        return False
    sock = socket or socket_name()
    ...
    if proc.returncode == 0:
        return True
    text = (stderr or b"").decode(errors="replace").strip()
    if "no server running" in text or "can't find session" in text:
        return False
    logger.warning(
        "tmux resize-window socket=%r session=%r %dx%d failed: %s",
        sock,
        session,
        cols,
        rows,
