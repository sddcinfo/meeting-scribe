...
_GUEST_CONST_SALT: Final[bytes] = b"scribe-guest-cookie-salt"


def derive_cookie_subkey(
    master_secret: bytes,
    boot_session_id: bytes | None,
    auth_version: int = 1,
    *,
    info: bytes = b"scribe-cookie-v36",
) -> bytes:
    """Derive a 32-byte HMAC subkey for cookie signing.

    Two rotation knobs participate:

    * ``boot_session_id`` salts the HKDF extract step. As of the
      session-persistence change in v2.0.x, ``state.boot_session_id``
      is loaded from disk (``~/.config/meeting-scribe/session-salt``)
      and only generated on first boot, so the salt — and therefore
      the derived subkey — is stable across restarts. Admin cookies
      issued before a restart continue to verify. To force-rotate
      the salt, delete the file and restart; ``factory_reset`` is the
      operator-friendly equivalent (it bumps ``auth_version`` below
      AND wipes the salt).
      Pass ``None`` (the guest cookie's case) to use a constant
      salt; that path was always restart-stable and is unchanged.

    * ``auth_version`` mixes into the HKDF info field as suffix
      bytes. Bumping the on-disk auth-version (factory_reset) rotates
      both signers in lockstep — both admin AND guest cookies stop
      verifying — without requiring a restart.

    ``info`` separates admin from guest in the signer namespace:
    ``b"scribe-cookie-v36"`` for admin (default), ``b"scribe-guest-
    cookie-v1"`` for guest. Within either namespace, two cookies
    issued at different ``auth_version``s have different HMACs.
    """
    salt = boot_session_id if boot_session_id is not None else _GUEST_CONST_SALT
    prk = _hkdf_extract(salt=salt, ikm=master_secret)
    versioned_info = info + b"|v=" + str(auth_version).encode()
    return _hkdf_expand(prk, info=versioned_info, length=32)
...
# Loopback-only kiosk cookie name. Issued by ``/kiosk-bootstrap`` to
# the cage chromium session running on the GB10 itself; bound to the
# ``Role.KIOSK`` principal which can read kiosk-allowlisted endpoints
# but never PUT/POST/DELETE on the admin write surface.
KIOSK_COOKIE_NAME: Final[str] = "scribe_kiosk"
...
# Distinct HMAC info for the kiosk cookie so an admin-cookie value
# cannot be replayed as a kiosk cookie (or vice-versa) even though both
# signers share the boot-derived subkey. Bump the version suffix
# whenever the cookie payload shape changes.
_KIOSK_COOKIE_HMAC_INFO: Final[bytes] = b"scribe-kiosk-cookie-v1"


@dataclass
class CookieSigner:
    """Persistent-keyed HMAC-signed cookie: ``<issued_unix>.<session_id>.<hex-hmac>``.

    The HMAC is computed over ``f"{issued_unix}|{session_id}"`` using a
    subkey derived from a persistent on-disk salt (see
    :func:`derive_cookie_subkey`). Three complementary invalidation
    primitives are layered:

    * **factory_reset** bumps ``auth_version``, which rotates the
      derived subkey — every previously-issued cookie fails HMAC verify
      from the next request.
    * **Logout** adds the cookie's ``session_id`` to
      ``state._revoked_sessions``; :meth:`verify` consults the optional
      ``is_revoked`` callable on every check.
    * **Expiry** — every cookie carries an issued-at timestamp; cookies
      older than ``max_age_seconds`` fail :meth:`verify`.

    Restart used to rotate the per-boot session salt and therefore
    every cookie; that was changed to a persistent-on-disk salt for UX
    reasons (see ``server.py`` rationale). Operators no longer get
    kicked out by a service restart.
