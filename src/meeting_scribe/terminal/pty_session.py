...
class TerminalSession:
    ...
    async def resize(self, cols: int, rows: int) -> None:
        ...
        # When the child is tmux, follow up with an explicit resize-window
        # so window-size=manual honours this PTY's new geometry. Without
        # this, the SIGWINCH from the ioctl alone would not change the
        # window size — that's the entire point of the manual policy:
        # "no implicit resize from any client". The argv check skips this
        # for the SCRIBE_TERM_SHELL test path (argv[0] is /bin/sh etc.).
        argv = getattr(self.proc, "args", None)
        if isinstance(argv, list | tuple) and argv and str(argv[0]).endswith("tmux"):
            from meeting_scribe.terminal.tmux_helper import resize_window

            ok = await resize_window(self.tmux_session, cols, rows)
            if not ok:
                logger.debug(
                    "tmux resize-window did not honour resize for %s -> %dx%d",
                    self.tmux_session,
                    cols,
                    rows,
                )
