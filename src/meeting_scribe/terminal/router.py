...
Every HTTP endpoint stacks ``require_admin`` (subnet + cookie). The
WS handler runs the same gate via ``require_admin_ws`` plus the
...
from meeting_scribe.terminal.auth import CookieSigner, TicketStore
...
def _origin_ok(ws: WebSocket) -> bool:
    ...
    We do NOT hard-code https: ``require_admin_ws`` already rejects
    off-AP and unauthenticated callers. Treating HTTP-on-LAN as
    forbidden here would break local browser testing without adding
    real security on top of subnet + cookie + ticket.
    """
...
def register_terminal_routes(app: FastAPI, cfg: TerminalRouterConfig) -> None:
    @app.post("/api/terminal/ticket", dependencies=[Depends(require_admin)])
    async def mint_ticket() -> JSONResponse:
        ticket = await cfg.ticket_store.mint()
        return JSONResponse({"ticket": ticket, "expires_in": cfg.ticket_store.ttl_seconds})

    @app.get("/api/terminal/sessions", dependencies=[Depends(require_admin)])
    ...
    @app.post("/api/terminal/tmux/kill", dependencies=[Depends(require_admin)])
    async def kill_tmux() -> JSONResponse:
        killed = await tmux_helper.kill_server()
        return JSONResponse({"killed": killed})

    @app.post(
        "/api/terminal/sessions/{name}/reset",
        dependencies=[Depends(require_admin)],
    )
    async def reset_session(name: str) -> JSONResponse:
...
async def _handle_ws(websocket: WebSocket, cfg: TerminalRouterConfig) -> None:
    # 1+2. Subnet + cookie via the shared gate. Returns False after
    # closing the WS with the appropriate code; we just exit.
