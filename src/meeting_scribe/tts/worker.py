...
#
# GB10 observations from 2026-05-08: the reliable baseline Qwen3-TTS path
# commonly takes 6-8s and can exceed that under GPU/memory pressure. Keep the
# default bounded, but give the backend enough room to deliver instead of
# falsely declaring "TTS stalled" on useful segments. Deployments can tighten
# this with SCRIBE_TTS_MAX_SPEECH_LAG_S.
_TTS_MAX_SPEECH_LAG_S = float(os.environ.get("SCRIBE_TTS_MAX_SPEECH_LAG_S", "15.0"))
...
# [P1-1-i2].
#
# Bumped 2026-05-08 for the GB10 baseline backend. Short utterances were
# timing out at 8s even though the containers were still making progress.
_TTS_SYNTH_TIMEOUT_BASE_S = float(os.environ.get("SCRIBE_TTS_SYNTH_TIMEOUT_BASE_S", "12.0"))
_TTS_SYNTH_TIMEOUT_PER_CHAR_S = float(os.environ.get("SCRIBE_TTS_SYNTH_TIMEOUT_PER_CHAR_S", "0.05"))
...
def _text_matches_tts_target_language(text: str, target_lang: str) -> bool:
    """Fail closed on obvious translation-language mismatches.

    The room sink is audible to everyone, so a bad language route is worse than
    a dropped segment. This guard only handles script-separable cases; Latin
    same-script pairs still rely on the translator/ASR labels.
    """
    ...
    if script_lang == "unknown":
        return True
    if target == "en":
        return script_lang == "en"
    if target == "ja":
        # Kanji-only Japanese fragments look like zh to the script heuristic.
        return script_lang in {"ja", "zh"}
    if target == "zh":
        return script_lang == "zh"
    if target == "ko":
        return script_lang == "ko"
    if target in {"ru", "uk"}:
        return script_lang == "ru"
    if target in {"ar", "th", "hi"}:
        return script_lang == target
    return True


def _is_tts_translator_meta_response(text: str) -> bool:
    """Detect translator refusal/apology text that must never be spoken."""
    ...
    if "does not appear" in normalized and (
        "meaningful" in normalized or "translatable" in normalized or "translation" in normalized
    ):
        return True
    if "doesn't appear" in normalized and (
        "meaningful" in normalized or "translatable" in normalized or "translation" in normalized
    ):
        return True
    return "cannot translate" in normalized or "can't translate" in normalized
...
def _flush_queued_tts_after_timeout() -> int:
    """Drop queued TTS after a backend timeout to avoid stale cascades."""
    ...
    while True:
        try:
            ...
            dropped += 1
    if dropped:
        state.metrics.tts_dropped_recovery_flush += dropped
        logger.warning("TTS recovery flush: dropped %d queued segment(s) after timeout", dropped)
    return dropped
...
def _start_tts_worker() -> None:
    ...
    if state.tts_dispatch_gate is None:
        # Token queue shared with the speakerphone button-feedback path.
        # Translation workers acquire a token before each synthesize call;
        # feedback uses ``get_nowait()`` for a true atomic try-acquire and
        # drops if the gate is empty. The container-concurrency constant
        # is the same as the semaphore so the two primitives stay in sync
        # for systems that have N>1 (today N=1, so this is one token).
        state.tts_dispatch_gate = asyncio.Queue(
            maxsize=state.TTS_CONTAINER_MAX_CONCURRENCY,
        )
        for _ in range(state.TTS_CONTAINER_MAX_CONCURRENCY):
            state.tts_dispatch_gate.put_nowait(object())
...
async def _do_tts_synthesis(event, speaker_id, deadline: float) -> None:
    ...
    if _is_tts_translator_meta_response(text):
        logger.warning(
            "TTS drop translator-meta seg=%s source_lang=%s target_lang=%s text=%r",
            event.segment_id,
            getattr(event, "language", ""),
            _norm_lang(event.translation.target_language),
            text[:160],
        )
        state.metrics.tts_dropped_translator_meta += 1
        return

    # Cap text to ~30 seconds of audio (~100 words ≈ 200 chars).
    MAX_TTS_CHARS = int(os.environ.get("SCRIBE_TTS_MAX_CHARS", "180"))
    ...
    if not _text_matches_tts_target_language(text, target_lang):
        logger.warning(
            "TTS drop language-mismatch seg=%s source_lang=%s target_lang=%s text=%r",
            event.segment_id,
            getattr(event, "language", ""),
            target_lang,
            text[:80],
    ...
    for voice_mode in voice_modes_needed:
        ...
        used_fallback = False
        logger.info(
            "TTS synthesize: seg=%s mode=%s speaker_id=%s fallback=%s "
            "source_lang=%s target_lang=%s studio_voice=%s chars=%d",
            event.segment_id,
            voice_mode,
            speaker_id,
            used_fallback,
        ...
        try:
            ...
            chosen_tts_url = None
            choose_url = getattr(state.tts_backend, "choose_url_for_request", None)
            if callable(choose_url):
                chosen_tts_url = choose_url()
            ...
            try:
                ...
                async def _run_stream():
                    ...
                    if callable(choose_url):
                        synth_kwargs["target_url"] = chosen_tts_url
                    # Acquire a token from the shared dispatch gate so
                    # the speakerphone button-feedback path can do an
                    # atomic non-blocking try-acquire and drop while
                    # this synth is in flight. The token is released in
                    # the ``finally`` below, even if synthesize_stream
                    # raises. See speakerphone/api.py::apply_speak.
                    if state.tts_dispatch_gate is None:
                        # Defensive: lifespan should have initialised this,
                        # but if a test path hasn't, fall through to the
                        # legacy semaphore-only flow.
                        gate_token = None
                    else:
                        gate_token = await state.tts_dispatch_gate.get()
                    try:
                        async for chunk in state.tts_backend.synthesize_stream(**synth_kwargs):
                            if len(chunk) == 0:
                                continue
                            if first_chunk_ms is None:
                                first_chunk_ms = (time.monotonic() - synth_start) * 1000.0
                            streamed_chunks.append(chunk)
                            # Cooperative yield between chunks: lets ASR /
                            # translation reclaim the loop during long synths.
                            # Loop-lag traces from 2026-05-08 09:08 showed this
                            # frame at 17.7 s; even a no-op sleep(0) breaks the
                            # back-to-back chunk hot loop.
                            await asyncio.sleep(0)
                    finally:
                        if gate_token is not None and state.tts_dispatch_gate is not None:
                            state.tts_dispatch_gate.put_nowait(gate_token)

                await asyncio.wait_for(_run_stream(), timeout=effective_timeout)
            except TimeoutError:
                logger.warning(
                    "TTS stream TIMEOUT seg=%s url=%s chars=%d timeout=%.2fs waited=%.2fs",
                    event.segment_id,
                    chosen_tts_url or "?",
                    len(text),
                    effective_timeout,
                ...
                mark_timeout = getattr(state.tts_backend, "mark_synthesis_timeout", None)
                if callable(mark_timeout):
                    mark_timeout(chosen_tts_url, effective_timeout)
                state.metrics.tts_synth_timeouts += 1
                _flush_queued_tts_after_timeout()
                return
        finally:
            state.tts_in_flight -= 1
        ...
        if state._audio_out_clients:
            await _send_audio_to_listeners(audio, target_lang, voice_mode, event, speaker_id)
