...
async def _call_vllm_summary(
    ...
    async with httpx.AsyncClient(base_url=vllm_url, timeout=timeout) as client:
        ...
        try:
            ...
            # Some transport-level exceptions stringify to '' (e.g. a
            # `RemoteProtocolError` raised when vLLM closes the keep-alive
            # connection mid-stream), which makes the resulting
            # "LLM call failed:" log unactionable. Include the type name
            # plus repr, and — for HTTP status errors — the response
            # status + a body snippet, so the next failure tells us
            # which layer dropped the call.
            etype = type(e).__name__
            detail = repr(e)
            with contextlib.suppress(Exception):
                if isinstance(e, httpx.HTTPStatusError):
                    body = (e.response.text or "").strip()
                    detail = f"{detail} status={e.response.status_code} body={body[:200]!r}"
            msg = f"LLM call failed [{etype}]: {detail}"
            logger.warning("vLLM summary call failed: %s", msg)
            return None, msg, elapsed_ms, model
...
async def generate_draft_summary(
    ...
    Uses priority -5 — just above translation's -10. Translation still
    preempts under contention so real-time UX is untouched, but TTS /
    furigana no longer block draft generation entirely. Bumped from +5
    in A2 step 3 after meeting `3db4286e-...` showed the previous
    priority left the draft far behind the actual transcript at stop
    time (531 / 1565 events).
