...
def _is_low_value_english_fragment(text: str) -> bool:
    """Return true for tiny English ASR fragments that add more noise than value.

    Qwen3-ASR sometimes emits one-word English snippets from short Japanese
    audio tails, especially after live input has fallen behind. Dropping only
    ultra-short Latin fragments is a conservative quality tradeoff: it removes
    pathological translation/TTS churn while preserving complete English turns.
    """
    lower = text.lower().strip()
    compact = re.sub(r"[^a-z']+", "", lower)
    if compact in _LOW_VALUE_ENGLISH_FRAGMENTS:
        return True
    words = re.findall(r"[a-z']+", lower)
    return len(words) == 1 and len(words[0]) <= 3
