...
can point at one or more endpoints (comma-separated).
...
_USE_STREAM_ENDPOINT = os.environ.get("SCRIBE_TTS_USE_STREAM_ENDPOINT", "1") != "0"
...
class Qwen3TTSBackend(TTSBackend):
    """Qwen3-TTS via faster-qwen3-tts with optional multi-endpoint pool.
    ...
    single endpoint caps at one synthesis at a time and pegs that core at
    100 % during decode. ``vllm_url`` accepts a comma-separated list for
    process-level parallelism, but the GB10 production profile intentionally
    runs one resident TTS endpoint because the full ASR + translation +
    diarization + TTS stack can CUDA-OOM with two local TTS models loaded.

    **Per-endpoint failure tracking**: a single endpoint entering a
    ...
    endpoint — so the pool never flagged the dead endpoint and kept
    ...
    def __init__(self, vllm_url: str | None = None) -> None:
        ...
        # After a supervisor-driven container restart, faster-qwen3-tts
        # needs tens of seconds to reload the model. During that warmup,
        # synthesis attempts may see connection failures. Those should
        # drop the current utterance, but they must not immediately mark
        # the backend degraded and cause a restart loop.
    ...
    def _next_url(self) -> str | None:
        """Pick the next URL via round-robin, skipping quarantined endpoints.

        If a quarantined endpoint has been parked for longer than
        ...
        if not eligible:
            ...
            # as a "best-effort try" so a fully quarantined endpoint set
            # still produces a loud request failure instead of silent drops.
        ...
        # an experimental endpoint pool is configured.
    ...
    def _mark_url_failure(self, url: str, err: str) -> None:
        ...
        self._refresh_degraded_from_pool()

    def _mark_url_timeout(self, url: str, timeout_s: float) -> None:
        """Quarantine a URL after the app-side realtime deadline expires.

        A synthesis that exceeds the worker deadline may keep running inside
        the TTS container even after the client coroutine is cancelled. Park
        that slow/wedged endpoint immediately so later recovery paths can make
        an explicit decision instead of feeding more work into it.
        """
        self._url_failures[url] = max(
            self._url_failures.get(url, 0),
            self.MAX_CONSECUTIVE_FAILURES,
        )
        if url not in self._quarantined:
            logger.error(
                "TTS replica quarantined: %s after app timeout %.2fs",
                url,
                timeout_s,
            )
        self._quarantined[url] = time.monotonic()
        self._last_error = f"{url} app timeout after {timeout_s:.2f}s"
        self._refresh_degraded_from_pool()
    ...
    def _refresh_degraded_from_pool(self) -> None:
        """Mark backend degraded only when every configured URL is parked."""
        self._degraded = bool(self._urls) and all(url in self._quarantined for url in self._urls)

    def choose_url_for_request(self) -> str | None:
        """Reserve the URL a caller should use for the next synthesis request.

        The worker uses this so an app-side timeout can be attributed to the
        exact replica that was selected. Direct callers can keep using
        synthesize_stream without passing a URL.
        """
        return self._next_url() or self._vllm_url

    def mark_synthesis_timeout(self, url: str | None, timeout_s: float) -> None:
        """Record that a worker-level synthesis timeout occurred."""
        if not url:
            self._last_error = f"app timeout after {timeout_s:.2f}s"
            self._degraded = True
            return
        self._mark_url_timeout(url, timeout_s)

    async def _wait_for_url_healthy(self, url: str, timeout_s: float = 35.0) -> bool:
        """Wait briefly for a TTS container restart after a 5xx/transport failure."""
        ...
        while time.monotonic() < deadline:
            try:
                ...
                pass  # container still restarting; retry until deadline
            await asyncio.sleep(1.0)
        return False

    async def reset_failure_state_after_restart(self) -> None:
        """Clear replica failure state after an external container restart.

        The faster-qwen3-tts container can keep returning HTTP 200 from
        /health while synthesis fails with CUDA errors. Recovery must come
        from the supervisor restarting the container(s), not from a shallow
        health probe. After that restart has been issued, reset the routing
        state so the endpoint set can be probed again with a fresh client.
        """
        ...
        self._restart_grace_until = time.monotonic() + float(
            os.environ.get("SCRIBE_TTS_RESTART_GRACE_SECONDS", "45")
        )
    ...
    async def start(self) -> None:
        ...
        self._mode = "vllm"
        if len(alive) == 1:
            logger.info("TTS: ready at %s (single endpoint)", alive[0])
        else:
            logger.info("TTS: ready, %d endpoints in pool: %s", len(alive), ", ".join(alive))
    ...
    async def synthesize_stream(
        ...
        _retry_after_restart: bool = True,
        ...
        # Pick a replica per request via round-robin. Production usually has
        # one URL; experimental TTS pools can provide more.
        target_url = target_url or self.choose_url_for_request()
        ...
        path = "/v1/audio/speech/stream" if _USE_STREAM_ENDPOINT else "/v1/audio/speech"
        try:
            async with self._http_client.stream("POST", f"{target_url}{path}", json=body) as resp:
                resp.raise_for_status()
                total_samples = 0
                if path.endswith("/stream"):
                    async for samples in self._iter_framed_pcm(resp):
                    ...
                    if residual:
                        logger.debug(
                            "TTS: dropping %d trailing byte(s) at stream end", len(residual)
                        )
            ...
            restartable = isinstance(e, httpx.TransportError) or (
                status_code is not None and status_code >= 500
            )
            if restartable and _retry_after_restart:
                logger.warning(
                    "TTS synthesis failed on %s with restartable error; waiting for "
                    "container recovery and retrying once: %s",
                    target_url,
                    e,
                )
                if self._http_client is not None and not self._http_client.is_closed:
                    await self._http_client.aclose()
                self._http_client = None
                healthy = await self._wait_for_url_healthy(target_url, timeout_s=35.0)
                if healthy:
                    async for chunk in self.synthesize_stream(
                        text=text,
                        language=language,
            ...
            if self._in_restart_grace():
                self._last_error = str(e)
                logger.warning(
                    "TTS synthesis failed during restart grace on %s; "
                    "dropping utterance without degrading backend: %s",
                    target_url,
                    e,
                )
                return
    ...
    @staticmethod
    async def _iter_framed_pcm(resp: httpx.Response) -> AsyncIterator[np.ndarray]:
        """Yield int16 PCM arrays from /v1/audio/speech/stream frames."""
        buf = bytearray()
        ...
        async for part in resp.aiter_bytes():
            ...
            while True:
                if expected is None:
                    if len(buf) < 4:
                        break
                    expected = struct.unpack(">I", bytes(buf[:4]))[0]
                    del buf[:4]
                    if expected == 0:
                        return
                if len(buf) < expected:
                ...
                if len(frame) % 2:
                    frame = frame[:-1]
                if frame:
                    yield np.frombuffer(frame, dtype=np.int16)
