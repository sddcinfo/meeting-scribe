...
class SortformerBackend(DiarizeBackend):
    ...
    def __init__(
        ...
        # When pyannote trips a CUDA/cuDNN failure, /health can still return
        # 200 while inference keeps failing. Back off live rolling diarization
        # for a short window so ASR/TTS/translation keep the GPU and event loop
        # responsive while the recovery monitor restarts or reprobes it.
        self._next_retry_ts: float = 0.0
        self._degraded_retry_cooldown_s: float = 15.0
    ...
    async def check_health(self) -> bool:
        ...
        if self._client is None:
            return False
        if self._degraded and time.monotonic() < self._next_retry_ts:
            return False
        try:
    ...
    async def process_audio(
        ...
        try:
            ...
            if self._consecutive_failures >= self.MAX_CONSECUTIVE_FAILURES:
                self._degraded = True
                self._next_retry_ts = time.monotonic() + self._degraded_retry_cooldown_s
                logger.error(
                    "Sortformer diarization marked degraded after %d failures; "
                    "backing off %.0fs: %s",
                    self._consecutive_failures,
                    self._degraded_retry_cooldown_s,
                    e,
                )
            else:
                logger.warning(
