...
def _http_verify() -> str:
    """Return the ``verify=`` argument for httpx.AsyncClient.

    Always returns the on-disk cert path. ``_server_url`` already gates
    the scheme on cert presence — when no cert exists the URL is
    ``http://...`` and ``verify=`` is irrelevant; when a cert does
    exist we want httpx to validate against it. CodeQL's
    ``py/request-without-cert-validation`` taint check is satisfied
    because the value is unconditionally a string path, never
    ``False``.
    """
    project_root = Path(__file__).resolve().parents[2]
    return str(project_root / "certs" / "cert.pem")
...
async def _phase_status_latency(config: ServerConfig) -> PhaseResult:
    ...
    try:
        async with httpx.AsyncClient(timeout=_HTTP_SLOW, verify=_http_verify()) as c:
            for _ in range(3):
                tt = time.monotonic()
                r = await c.get(url)
    ...
    *with the python-multipart assertion*, which is what we're
    ...
    try:
        async with httpx.AsyncClient(
            timeout=_HTTP_UPLOAD, verify=_http_verify(), cookies=cookies
        ) as c:
...
async def _phase_meeting_qr(config: ServerConfig) -> PhaseResult:
    ...
    * /api/meeting/wifi returning 503 ``Hotspot not active`` after
    ...
    try:
        async with httpx.AsyncClient(
            timeout=_HTTP_FAST, verify=_http_verify(), cookies=cookies
        ) as c:
