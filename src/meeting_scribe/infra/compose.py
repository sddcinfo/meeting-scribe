...
def assert_recipe_source_parity() -> list[RecipeMismatch]:
    ...
    # TTS — GB10 reliability mode runs a single resident faster-qwen3-tts
    # container. The app backend still supports comma-separated pools, but
    # compose/recipe parity should describe only the default resident service.
    tts = services.get("qwen3-tts") or {}
    tts_env = _extract_compose_env(tts.get("environment"))
    if tts_env:
        recipe = load_recipe("tts")
        if "TTS_PORT" in tts_env:
            cv, rv = tts_env["TTS_PORT"], recipe.get("port")
            if rv is not None and int(cv) != int(rv):
                mismatches.append(RecipeMismatch("qwen3-tts", "TTS_PORT", cv, str(rv)))
        if "TTS_MODEL" in tts_env:
            cv, rv = tts_env["TTS_MODEL"], recipe.get("model_id")
            if rv is not None and cv != str(rv):
                mismatches.append(RecipeMismatch("qwen3-tts", "TTS_MODEL", cv, str(rv)))
