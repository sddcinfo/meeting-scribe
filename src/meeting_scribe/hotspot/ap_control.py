...
# wifi driver needs a beat between ``con down`` and ``con up`` —
# without it ``con up`` races the down and supplicant times out
# associating. Module-level so the unit-test suite can monkeypatch
# this to 0 instead of stalling on the real 2-second sleep on every
# rotation test (10+ seconds of test time pre-fix).
_AP_BOUNCE_DRIVER_SETTLE_S: float = 2.0

# Shared with the rotation self-heal path. Imported via ``_wifi.``
# lookup at call time so test monkeypatch on ``meeting_scribe.wifi.X``
# affects both bring-up and rotation in lockstep.
_recreate_ap_profile_with_sae = _wifi._recreate_ap_profile_with_sae

logger = logging.getLogger(__name__)

# Rotation post-condition state. Set on every meeting-start AP rotation
# so ``/api/status`` can surface "rotation_ok=false" + the live vs target
# SSID delta until the next successful rotation. Without this surface,
# the rotation mismatch only logged a WARN and the operator had no way
# to know the AP wasn't actually rotating.
_LAST_ROTATION_TARGET_SSID: str | None = None
_LAST_ROTATION_LIVE_SSID: str | None = None
_LAST_ROTATION_AT: float | None = None
_LAST_ROTATION_OK: bool = True


def get_last_rotation_state() -> dict[str, object]:
    """Snapshot of the most recent AP rotation outcome.

    Consumed by ``/api/status`` to render the ``hotspot`` block. Safe
    to call at any point — defaults to ``rotation_ok=True`` when no
    rotation has ever happened in this process.
    """
    return {
        "rotation_ok": _LAST_ROTATION_OK,
        "target_ssid": _LAST_ROTATION_TARGET_SSID,
        "live_ssid": _LAST_ROTATION_LIVE_SSID,
        "last_rotation_at": _LAST_ROTATION_AT,
    }


def _record_rotation_outcome(*, target: str | None, live: str | None, at: float) -> None:
    """Persist the rotation outcome for ``/api/status`` to consume."""
    import time as _time

    global _LAST_ROTATION_TARGET_SSID, _LAST_ROTATION_LIVE_SSID
    global _LAST_ROTATION_AT, _LAST_ROTATION_OK
    _LAST_ROTATION_TARGET_SSID = target
    _LAST_ROTATION_LIVE_SSID = live
    _LAST_ROTATION_AT = at or _time.time()
    _LAST_ROTATION_OK = bool(target) and bool(live) and target == live
...
async def _start_wifi_ap(meeting_id: str | None = None) -> None:
    ...
    def _rotate_profile_and_bounce() -> subprocess.CompletedProcess[str] | None:
        ...
        Self-heals the "key-mgmt: property is missing" rejection mode
        that hits when an existing profile carries inconsistent
        security args from a prior config. Deletes + re-adds the
        connection from scratch using the same WPA3-SAE arg set the
        first-run bootstrap uses (``wifi._bring_up_ap``), then proceeds
        with the bounce. The 2026-05-14 demo failure was exactly this:
        the DellDemo-AP profile had stale wifi-sec state, every meeting
        rotation logged the WARN but NM kept serving the previous SSID.
        """
        import time as _time
        ...
        if modify.returncode != 0:
            stderr = (modify.stderr or "").lower()
            if "key-mgmt: property is missing" in stderr:
                logger.warning(
                    "WiFi AP rotation hit stale-security state — self-healing "
                    "by recreating the profile from scratch (target ssid=%s)",
                    new_ssid,
                )
                recreated = _recreate_ap_profile_with_sae(new_ssid, new_password)
                if recreated is None or recreated.returncode != 0:
                    return recreated or modify
            else:
                return modify

        _wifi._run_nmcli_sync(["con", "down", AP_CON_NAME], timeout=_NMCLI_CON_DOWN_TIMEOUT)
        # wifi driver needs a beat between down and up; pulled out as
        # a module-level constant so the unit-test suite can monkeypatch
        # it to 0 without poking at the production sleep call.
        _time.sleep(_AP_BOUNCE_DRIVER_SETTLE_S)
    ...
    if await loop.run_in_executor(None, _wifi._write_hotspot_state_sync):
        # Post-condition read MUST bypass the TTL cache: the cache was
        # invalidated by the writes above, but a 5-second-stale value
        # from earlier in this rotation would mask the very mismatch
        # we're checking for. Pass ``bypass_cache=True`` for a fresh
        # nmcli read.
        creds = await loop.run_in_executor(
            None,
            lambda: _wifi._nmcli_read_live_ap_credentials(bypass_cache=True),
        )
        live_ssid = creds[0] if creds else None
        logger.info("Hotspot state written from live nmcli: ssid=%s", live_ssid or "<unknown>")
        # Persist the outcome so ``/api/status`` can render it for the
        # admin SPA banner, regardless of whether the SSID matched.
        import time as _time

        _record_rotation_outcome(target=new_ssid, live=live_ssid, at=_time.time())
        if live_ssid and live_ssid != new_ssid:
            # Per-meeting SSID rotation is a core demo feature; silently
            # serving the previous profile is a customer-visible failure.
            logger.error(
                "Live AP ssid %r does not match rotation target %r — "
                "rotation failed; NM is serving the previous profile",
                live_ssid,
                new_ssid,
            )
    else:
        ...
        _record_rotation_outcome(target=new_ssid, live=None, at=_time.time())
...
async def _apply_hotspot_firewall() -> None:
    """Async wrapper around ``wifi._apply_simple_firewall``.

    Setup-mode and operating-mode share one AP-iface-scoped INPUT +
    FORWARD allowlist. ``state.config.port`` is not consulted —
    admin HTTPS binds at ``10.42.0.1:443`` only, there is no separate
    admin port.
    """
    await asyncio.get_event_loop().run_in_executor(None, _wifi._apply_simple_firewall)


async def _remove_hotspot_firewall() -> None:
    """Remove every ``--comment ms-fw`` rule on both v4 + v6.

    Legacy ``meeting-scribe-hotspot`` rules are also cleared via
    ``_wifi._remove_firewall`` so a stale install doesn't leave
    duplicate ACCEPT/REJECT pairs after upgrade.
    """

    def _drop_all() -> None:
        _wifi._ms_fw_remove("iptables")
        _wifi._ms_fw_remove("ip6tables")
        _wifi._remove_firewall()

    await asyncio.get_event_loop().run_in_executor(None, _drop_all)
