...
from meeting_scribe.server_support.qr import (
    _qr_svg,
    _wifi_qr_svg,
)
...
async def get_wifi_info(request: fastapi.Request) -> JSONResponse:
    ...
    # The captive HTTP sub-app on port 80 hosts this portal; the URL
    # uses http:// (no port) so:
    #   - No self-signed cert warnings when guests scan and join
    #   - iOS' captive mini-browser actually follows the URL
    # The captive sub-app's catch-all 308 then promotes the guest to
    # https://<ap_ip>/ where the cookie-driven gate runs.
    ...
    from meeting_scribe.cli._common import appliance_pin as _appliance_pin

    # QR fields kept in the payload (empty SVG stubs) so legacy
    # validate_customer probes don't 500. The user-facing UI no longer
    # renders QR codes; the 4-digit ``appliance_pin`` replaces them.
    return JSONResponse(
        {
            "ssid": ssid,
            "password": password,
            ...
            # 4-digit appliance pin — same number that appears in the
            # SSID suffix and as the guest auth PIN. Now the primary
            # join hint; QR codes are no longer rendered.
