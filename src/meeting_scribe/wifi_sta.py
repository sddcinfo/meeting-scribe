...
async def sta_iface_ensure(*, phy: str = PHY_DEFAULT) -> bool:
    ...
    ``phy`` if it doesn't already exist, and bring it up.

    ``iw phy ... interface add`` requires CAP_NET_ADMIN; we wrap it in
    sudo so the helper works whether invoked from the server process
    (already root) or from a user-shell CLI (sudo propagates the
    capability). The freshly-added interface is then brought
    ``up`` so subsequent scans actually have a usable netdev.

    Returns True iff the interface is present + up after the call.
    """
    if not sta_iface_present():
        proc = await _run(
            ["sudo", "iw", "phy", phy, "interface", "add", STA_IFACE, "type", "managed"]
        )
        if proc.returncode != 0:
            return False
    # New interfaces come up DOWN by default — bring it up so the
    # supplicant can scan + associate. Idempotent: ip link set up
    # on an already-up link is a no-op.
    await _run(["sudo", "ip", "link", "set", STA_IFACE, "up"])
