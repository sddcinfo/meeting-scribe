...
def _parse_context_directions(
    value: str | list[str] | tuple[str, ...],
) -> set[tuple[str, str]]:
    """Parse ``ja:en,en:ja`` style live-context direction config."""
    ...
    for raw in raw_parts:
        ...
        if ":" in part:
            ...
            logger.warning("Ignoring invalid live translate context direction: %r", raw)
            continue
        src = src.strip()
        dst = dst.strip()
...
def _translation_matches_target_language(text: str, target_lang: str) -> bool:
    """Fail closed on obvious wrong-script translations before TTS.

    This is deliberately script-only. Same-script language pairs still rely on
    the translator, but en/ja failures are separable and should be retried or
    dropped before room audio can speak the wrong language.
    """
    if text.rstrip().endswith(f"::{target_lang}"):
        return True
    ...
    if target_lang == "en":
        if script_lang == "en":
            return True
        # Allow short mixed-script outputs such as product names or test doubles
        # that carry an explicit English marker, but still reject CJK-only
        # translations before they can reach TTS.
        latin = sum(1 for ch in text if ("a" <= ch.lower() <= "z"))
        cjk = sum(1 for ch in text if "\u3040" <= ch <= "\u30ff" or "\u4e00" <= ch <= "\u9fff")
        return latin >= 2 and latin >= cjk
    if target_lang == "ja":
    ...
    if target_lang in {"ar", "th", "hi"}:
        return script_lang == target_lang
    return True
...
class TranslationQueue:
    ...
    def __init__(
        ...
        *,
        ...
        # Active meeting id — set by ``bind_meeting`` (or its alias
        ...
        # context history.  ``None`` when no meeting is bound (startup,
        # between meetings, stopped).
        ...
        # path.  Keyed by meeting_id first, then by ``(source_lang,
        # target_lang)`` direction.  Keyed-by-meeting structure makes
        # accidental cross-meeting reads structurally impossible —
        # there is no shared dict to leak across.  Each inner deque
        # is bounded by ``ServerConfig.live_translate_context_window_ja_en``.
        ...
        # Fragment-gated: when True, ``prior_context`` only attaches
        # to utterances that look fragmentary.  Default False; gets
        # flipped on per meeting via
        # ``bind_meeting(fragment_gated=True)``.
        self._fragment_gated: bool = False
        self._live_context_directions: set[tuple[str, str]] = {("ja", "en")}
    ...
    def bind_meeting(
        ...
        *,
        ...
        context_directions: str | list[str] | tuple[str, ...] = "ja:en",
    ) -> None:
        ...
        self._live_context_directions = _parse_context_directions(context_directions)
    ...
    # Alias for the older ``set_active_meeting_id`` name; both point
    # at the same ``bind_meeting`` behaviour.
    def set_active_meeting_id(self, meeting_id: str | None) -> None:
        self.bind_meeting(meeting_id)

    # Fragment gating for the live context window.  Short affirmatives
    # ("はい", "そうですね", "OK") don't benefit from context and they
    # are the LRU cache's best customers.  Routing context through them
    # just bypasses the cache for no quality win.  When
    # ``live_translate_context_fragment_gated`` is on, we only attach
    # ``prior_context`` to utterances that look fragmentary — short OR
    # ending in a Japanese particle where the ASR likely trailed off
    # mid-sentence.  Keeps the 15-20% cache hit rate on short
    # affirmatives (see ``backends/translate_vllm.py:99-107``).
    ...
    def _resolve_prior_context(
        ...
        Returns ``None`` unless ALL of:
          ...
          * direction is enabled by ``context_directions`` at meeting
            bind time;
        ...
        if self._live_history_maxlen <= 0:
            return None
        if (source_lang, target_lang) not in self._live_context_directions:
            return None
    ...
    def _append_live_history(
        ...
        not enabled by ``context_directions`` — the history is only
        exercised by directions that read it, so leaving other
        directions empty is the cheap default.  No-op when the queue is
        not bound to a meeting.
        """
        from collections import deque
    ...
    async def _translate_one(self, item: _WorkItem, target_lang: str, worker_id: int) -> None:
        ...
        try:
            ...
            if not _translation_matches_target_language(translated_text, target_lang):
                logger.warning(
                    "Worker %d wrong-script translation for segment_id=%s target=%s; retrying once",
                    worker_id,
                    item.segment_id,
                    target_lang,
                )
                ...
                if not _translation_matches_target_language(translated_text, target_lang):
                    logger.warning(
                        "Worker %d wrong-script translation persisted for segment_id=%s target=%s",
                        worker_id,
                        item.segment_id,
                        target_lang,
                    )
