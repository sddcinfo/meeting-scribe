...
def prefer_event(existing: dict, new: dict) -> dict:
    """Pick the canonical journal entry for one segment_id.

    The journal is append-only and carries multiple writes per segment:
    an ASR final, a speaker-correction revision, and one or more
    translation updates. ``with_translation`` in ``models.py`` reuses
    the source event's ``revision``, so the same segment can hold two
    journal lines at the same revision — one with ``translation: null``
    and one with the completed translation. A naive ``new.rev >
    existing.rev`` dedup drops whichever rev-tie line was written second,
    silently losing translations roughly 10% of the time. See the
    2026-05-14 audit of meeting 7ae7a0f2 (13/107 segments lost).

    Contract: highest revision wins as the base. Whichever side carries
    a non-empty translation gets its translation carried forward onto
    the base. On a revision tie, whichever side carries the translation
    wins; if both or neither have one, the later-written entry wins
    (matches journal append order).

    Pure: takes plain dicts, returns one of the inputs or a shallow
    copy. Callers are responsible for ``segment_id`` matching before
    invoking; this helper does not look at the id.
    """
    e_has_tr = bool((new.get("translation") or {}).get("text"))
    ex_has_tr = bool((existing.get("translation") or {}).get("text"))
    e_rev = new.get("revision", 0)
    ex_rev = existing.get("revision", 0)
    if e_rev > ex_rev:
        base = dict(new)
        if not e_has_tr and ex_has_tr:
            base["translation"] = existing["translation"]
        return base
    if ex_rev > e_rev:
        if not ex_has_tr and e_has_tr:
            base = dict(existing)
            base["translation"] = new["translation"]
            return base
        return existing
    # Equal revisions — translation-bearing entry wins; else later write.
...
def _generate_speaker_data(
    ...
    for line in journal_path.read_text().splitlines():
        ...
        best[sid] = e if existing is None else prefer_event(existing, e)
