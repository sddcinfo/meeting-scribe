...
async def _broadcast_json(data: dict) -> None:
    ...
    Role-aware fan-out: admin sockets receive every registered event;
    kiosk sockets (tracked in ``state.kiosk_ws_connections``) only
    receive events listed in ``KIOSK_ALLOWED_EVENT_TYPES``. This is the
    server-side enforcement of the "kiosk role never sees operator
    diagnostics" rule — the client is never trusted to filter.
    ...
    from meeting_scribe.ws.event_types import is_kiosk_allowed

    text = _json.dumps(data)
    type_value = data.get("type") if isinstance(data, dict) else None
    kiosk_allowed = is_kiosk_allowed(type_value)
    kiosk_set: set = getattr(state, "kiosk_ws_connections", set())

    dead: list[WebSocket] = []
    for ws in list(state.ws_connections):
        # Skip kiosk-cookie sockets when this event isn't on their
        # allowlist. The kiosk set membership is the source of truth;
        # presence in ws_connections doesn't imply ADMIN role.
        if ws in kiosk_set and not kiosk_allowed:
            continue
        try:
...
async def _broadcast(event: TranscriptEvent) -> None:
    ...
    Reaches admin audio WS, view-only WS (popout + guest), kiosk
    socket, and hotspot guests since they all share the
    ``state.ws_connections`` set. Frontends look up the block by
    ``segment_id`` and update in place.

    Transcript events are always allowed for kiosks; the popout mirror
    needs to render the transcript flow to do its job.
    """
    payload = event.model_dump(mode="json")
    if event.utterance_end_at is not None:
        # Keep the internal timestamp out of journals/model JSON by default,
        # but expose it on the live stream so the local e2e probe can measure
        # utterance-end -> translation-complete latency from the same event.
        payload["utterance_end_at"] = event.utterance_end_at
