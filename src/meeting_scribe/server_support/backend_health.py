...
async def _deep_check_tts() -> dict:
    ...
    if not urls:
        raw_url = (
            getattr(state.tts_backend, "_vllm_url", None) or state.config.tts_vllm_url or ""
        ).strip()
        ...
        async with httpx.AsyncClient(timeout=3.0) as c:
            for url in urls:
                try:
                    ...
                    failures.append(f"{url}: {type(e).__name__}: {str(e)[:60]}")
                    continue
                if r.status_code != 200:
                ...
                if isinstance(body, dict):
                    ...
                    if status and status != "healthy":
                        failures.append(f"{url}: health status={status}")
                        continue
                return {
                    "ready": True,
                    "detail": None if not failures else f"degraded pool: {'; '.join(failures)}",
                }
        return {"ready": False, "detail": "; ".join(failures) or "no healthy TTS endpoints"}
    except Exception as e:
        return {"ready": False, "detail": f"{type(e).__name__}: {str(e)[:80]}"}
