...
# loop.
_MAX_AUDIO_OUT_CLIENTS = int(os.environ.get("SCRIBE_AUDIO_OUT_MAX_CLIENTS", "32"))
...
_AUDIO_FORMAT_GRACE_S = float(os.environ.get("SCRIBE_AUDIO_FORMAT_GRACE_S", "1.0"))
# Max buffered audio seconds while waiting for `set_format`. Per-
# listener bound so a stuck handshake can't balloon memory.
_AUDIO_FORMAT_PENDING_CAP_S = float(os.environ.get("SCRIBE_AUDIO_FORMAT_PENDING_CAP_S", "1.0"))
...
def _create_audio_out_session(ws: WebSocket) -> ClientSession:
    ...
    transport_hint = query_params.get("transport", "") if hasattr(query_params, "get") else ""
    transport = "bt_headset" if transport_hint == "bt_headset" else "web_browser"
    if transport != "bt_headset" and has_admin_session(ws):
        transport = "admin_monitor"
    pref = ClientSession(
        send_audio=True,
