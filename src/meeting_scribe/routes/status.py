    ...
    return JSONResponse(
        {
            ...
            # ``connections`` is the recorder-socket count that the
            # client-side meeting reconciler reads to decide "is the
            # stale recorder still around?" (Case C) and "can I
            # upgrade to recorder now?" (upgrade branch). It MUST
            # exclude passive observers (view-only popouts + HDMI
            # kiosk mirror) or the admin tab wedges in view-only
            # mode forever - those sockets never go away. Tracked
            # by ``ws/audio_input.py`` via ``state.recorder_ws_connections``.
            "connections": len(getattr(state, "recorder_ws_connections", set())),
            "connections_total": len(state.ws_connections),
            "audio_out_connections": len(state._audio_out_clients),
            # When True, the live mic feed is the server-side PipeWire capture
            # (e.g. a paired Poly), NOT the browser's `getUserMedia` stream.
            # The meeting-page meter widget reads this to freeze the
            # browser-mic level bar and surface a "server mic active"
            # indicator — otherwise the bar misleads operators into thinking
            # ASR is hearing the laptop mic when it is actually being
            # dropped at the audio_input boundary.
            "server_mic_active_live": bool(getattr(state, "server_mic_active", False)),
            # Phase-1 mic auto-rebind / failure status. Values:
            #   "ok"              — routing resolved + capture running
            #   "ambiguous"       — multiple sources match persisted identity
            #   "unresolved"      — mic device not currently present
            #   "capture_failed"  — routing resolved but capture refused to start
            # Phase-5 meeting-start preflight rejects all non-"ok" values.
            "audio_route_status": getattr(state, "audio_route_status", "ok"),
            # Operator-facing banners surfaced via the ``.meeting-banner``
            # component. Only un-dismissed entries.
            "admin_notifications": _active_admin_notifications(),
            # Hotspot rotation surface. ``rotation_ok=False`` within
            # the last ~60 s means the admin UI should banner a red
            # "AP did not rotate" notice; ``True`` (or null) means
            # the AP is broadcasting the meeting's target SSID.
            "hotspot": _hotspot_rotation_state(),
...
def _active_admin_notifications() -> list[dict[str, Any]]:
    """Return un-dismissed admin notifications for the /api/status payload.

    Imported lazily to avoid an import cycle on module load
    (status.py is imported very early in the server bootstrap chain).
    """
    try:
        from meeting_scribe.server_support import admin_notifications

        return admin_notifications.active_notifications()
    except Exception:  # pragma: no cover — defensive, must not break /api/status
        return []


def _hotspot_rotation_state() -> dict[str, Any]:
    """Snapshot the last AP rotation outcome for /api/status.

    Lazy import same as ``_active_admin_notifications`` — the hotspot
    module pulls in nmcli + asyncio plumbing we don't want loaded just
    to render the status response on systems without WiFi.
    """
    try:
        from meeting_scribe.hotspot import ap_control

        return ap_control.get_last_rotation_state()
    except Exception:  # pragma: no cover — must not break /api/status
        return {
            "rotation_ok": True,
            "target_ssid": None,
            "live_ssid": None,
