...
from meeting_scribe.server_support.meeting_artifacts import prefer_event
            ...
            try:
                ...
                if summary_path.exists():
                    try:
                        ...
                        pass  # malformed summary.json — listing degrades to no preview, not a 500

                # Phase B progress sidecar — present while a meeting is
                # finalizing in the background or while a failure-terminal
                # state is awaiting Reprocess / Dismiss. Surfaced here
                # so the landing page can hydrate ``_bgFinalizeTrackers``
                # without waiting for a WS event after a page reload.
                phase_b_progress = None
                try:
                    phase_b_progress = state.storage.read_phase_b_progress(meta["meeting_id"])
                except Exception:
                    phase_b_progress = None
                ...
                results.append(
                    {
                        ...
                        "phase_b_progress": phase_b_progress,
                    }
                )
            except Exception:
                continue
...
_AUDIO_BYTES_PER_SEC = 16000 * 2  # s16le 16 kHz mono
_AUDIO_WAV_HEADER_BYTES = 44
_AUDIO_STREAM_CHUNK = 64 * 1024  # 64 KiB; 8x the prior 8 KiB to cut yield count


def _build_wav_header(pcm_size: int) -> bytes:
    sample_rate = 16000
    channels = 1
    bits_per_sample = 16
    return _struct.pack(
...
def _parse_byte_range(header_value: str, total_size: int) -> tuple[int, int] | None:
    """Parse an HTTP ``Range: bytes=...`` header.

    Returns ``(start, end)`` inclusive, or ``None`` if the header is
    malformed or out of range. Only the first range spec is honoured —
    multipart ranges are rare for ``<audio>`` elements and rejecting
    them keeps the streaming logic single-file.
    """
    prefix = "bytes="
    if not header_value.startswith(prefix):
        return None
    first_spec = header_value[len(prefix) :].split(",", 1)[0].strip()
    if "-" not in first_spec:
        return None
    start_s, _, end_s = first_spec.partition("-")
    try:
        if start_s == "":
            n = int(end_s)
            if n <= 0:
                return None
            return max(0, total_size - n), total_size - 1
        start = int(start_s)
...
async def get_meeting_audio(
    ...
    """Stream meeting audio as WAV with HTTP Range support.

    Query params (orthogonal to ``Range``): ``start_ms`` (default 0),
    ``end_ms`` (default full duration). These select the underlying
    audio span the WAV represents — used by the segment-playback path
    where the client wants a specific transcript span.

    ``Range: bytes=...`` selects a byte slice of the WAV response and
    returns ``206 Partial Content``. The PCM is streamed from disk in
    64 KiB chunks (no full-file read into memory) so seek-driven
    range requests on a 1+ hour meeting (~200 MB PCM) start playing
    immediately instead of pre-buffering the whole file.
    """
    ...
    # Clamp the ms span to what's actually on disk.
    start_ms = max(0, min(start_ms, total_duration))
    end_ms = max(start_ms, min(end_ms, total_duration))

    audio_offset_ms = state.storage.audio_offset_ms(meeting_id)
    pcm_start_byte = int(max(0, start_ms - audio_offset_ms) / 1000 * _AUDIO_BYTES_PER_SEC)
    pcm_end_byte = int(max(0, end_ms - audio_offset_ms) / 1000 * _AUDIO_BYTES_PER_SEC)
    pcm_size = pcm_end_byte - pcm_start_byte
    if pcm_size <= 0:
        return JSONResponse({"error": "No audio in range"}, status_code=404)

    header = _build_wav_header(pcm_size)
    total_size = _AUDIO_WAV_HEADER_BYTES + pcm_size
    ...
    if range_header:
        parsed = _parse_byte_range(range_header, total_size)
        if parsed is None:
            return Response(
                status_code=416,
    ...
    def stream():
        ...
        # WAV header portion of the response stream lives at bytes [0, 44).
        if cursor < _AUDIO_WAV_HEADER_BYTES:
            header_end_excl = min(end_inclusive + 1, _AUDIO_WAV_HEADER_BYTES)
            yield header[cursor:header_end_excl]
            cursor = header_end_excl
        # PCM portion lives at [44, total_size). Map response-byte cursor
        # back to PCM-file offset.
        if cursor <= end_inclusive:
            file_offset = pcm_start_byte + (cursor - _AUDIO_WAV_HEADER_BYTES)
            remaining = end_inclusive - cursor + 1
            with open(pcm_path, "rb") as f:
                f.seek(file_offset)
                while remaining > 0:
                    chunk = f.read(min(_AUDIO_STREAM_CHUNK, remaining))
                    if not chunk:
                        break
                    yield chunk
                    remaining -= len(chunk)
    ...
    if is_range_request:
        response_headers["Content-Range"] = f"bytes {range_start}-{range_end}/{total_size}"
...
async def get_meeting(meeting_id: str) -> JSONResponse:
    ...
    # Deduplicate by segment_id via the shared translation-preserving
    # helper. See ``prefer_event`` for the tie-breaking contract.
    best: dict[str, dict] = {}
    for event in events:
        ...
        best[sid] = event if existing is None else prefer_event(existing, event)
    ...
    # Phase B progress sidecar — drives the inline finalize banner on
    # the detail view. Either an in-flight snapshot (with eta_seconds)
    # or a failure-terminal state awaiting Reprocess / Dismiss. Absent
    # when the meeting has fully finalized.
    try:
        result["phase_b_progress"] = state.storage.read_phase_b_progress(meeting_id)
    except Exception:
        result["phase_b_progress"] = None
