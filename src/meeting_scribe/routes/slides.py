...
* ``/slides/source.pptx``, ``/slides/original.pdf``, and
  ``/slides/translated.pdf`` — file exports of the source PPTX,
  the lossless original-deck PDF, and the lossless translated-deck
  PDF (404s for monolingual decks).
...
@router.get("/api/meetings/{meeting_id}/slides/notes")
async def get_slide_notes(meeting_id: str):
    """Speaker-notes per slide (one string per slide, empty if blank).

    Source language only. Read from ``slide_notes.json`` under the
    deck's output dir, written by the convert pipeline at
    extracting_text time. Returns ``{"notes": [...]}`` so the admin
    "translation + presentation" view can fetch once and index by
    current slide as the operator navigates.
    """
    ...
    if not deck_id:
        return JSONResponse({"error": "Deck has no id"}, status_code=404)
    # _deck_dir is private but the only canonical path-builder; using
    # it here keeps us in lockstep with the rest of the routes that
    # read deck files.
    deck_dir = state.slide_job_runner._deck_dir(meeting_id, deck_id)
    notes_path = deck_dir / "slide_notes.json"
    if not notes_path.exists():
        # Older decks parsed pre-notes-extraction don't have this file.
        # Return empty list of the right length so the UI degrades to
        # "no notes" cleanly.
        total = int(meta.get("total_slides", 0))
        return JSONResponse({"notes": [""] * total})
    try:
        data = _json.loads(notes_path.read_text())
    except Exception:
        return JSONResponse({"error": "Notes file unreadable"}, status_code=500)
...
@router.get("/api/meetings/{meeting_id}/slides/translated.pdf")
async def get_slides_translated_pdf(meeting_id: str):
    """Serve the lossless PDF render of the translated deck. 404s for
    monolingual decks or when the post-express bulk render hasn't
    completed yet."""
    if state.slide_job_runner is None:
        return JSONResponse({"error": "Slides not enabled"}, status_code=503)

    pdf_path = state.slide_job_runner.get_translated_pdf_path(meeting_id)
    if pdf_path is None:
        return JSONResponse({"error": "Translated PDF not found"}, status_code=404)
