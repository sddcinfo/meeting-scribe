...
async def reprocess_meeting(
    ...
    try:
        ...
        # Additive — UI ignores unknown files.
    ...
    # These two helpers live in server_support.meeting_artifacts (moved
    # out of meeting_scribe.server during the v2.0.0 refactor). The
    # previous code path imported `meeting_scribe.server` and called
    # `srv._generate_speaker_data(...)` — the import succeeded but the
    # attributes silently went away, leaving every full-reprocess with
    # stale `timeline.json` + `speaker_lanes.json` while the new
    # `journal.jsonl` had freshly-generated segment_ids that didn't
    # match. Symptom on the UI side: clicking a diarize lane block
    # didn't move the transcript (segment_id lookup miss). The
    # try/except below stays so a regression doesn't silently corrupt
    # the meeting; failures here MUST be loud.
    ...
    try:
        ...
        _generate_timeline(meeting_dir.name, meeting_dir=meeting_dir)
    except Exception as e:
