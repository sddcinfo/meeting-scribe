...
class RecoverInterruptedResult(NamedTuple):
    """Triage outcome from :meth:`MeetingStorage.recover_interrupted`.

    Attributes:
        interrupted_count: Number of meetings transitioned to INTERRUPTED
            in this pass (RECORDING/FINALIZING with no resumable
            finalize_context, or REPROCESSING that crashed).
        finalizing_ids: Meetings whose Phase B should be resumed by the
            ``finalize_recovery.replay_pending_finalize`` driver.
        cleanup_ids: Meetings already at COMPLETE but with leftover
            finalize artifacts on disk (Phase B crashed between
            transition_state(COMPLETE) and _cleanup_finalize_context_files).
    """
...
# Names of files Phase A may have written before its `phase_a_done: true`
# rewrite. If the meeting goes to INTERRUPTED before that rewrite, these
# stale partials must be swept so a future Stop on a fresh meeting (or a
# reprocess on this one) doesn't see them and act on bad assumptions.
_PHASE_A_PARTIAL_ARTIFACTS: tuple[str, ...] = (
    "asr_tail.bin",
    "asr_tail.json",
    "eager_summary_cache.json",
    "pending_translations.jsonl",
    "finalize_context.json",
)


# Names of files Phase B may have written during its run. Used by the
# cleanup-only recovery path AND by the in-process Phase B success path
# at end-of-finalize. Removal is idempotent (missing_ok=True).
_PHASE_B_ARTIFACTS: tuple[str, ...] = (
    *_PHASE_A_PARTIAL_ARTIFACTS,
    "finalize_progress.json",
    "tail_events.jsonl",
    "tail_events.jsonl.applied_offset",
    "tail_translations.jsonl",
    ".diarize_attached",
)


def _sweep_phase_a_artifacts(meeting_dir: Path) -> None:
    """Remove every Phase A sidecar file from ``meeting_dir``.

    Idempotent — uses ``unlink(missing_ok=True)`` so concurrent calls
    don't race. Called from ``recover_interrupted`` when Phase A
    crashed mid-write (``phase_a_done: false`` on disk) so the meeting
    is left in a known-clean state before INTERRUPTED.
    """
    for name in _PHASE_A_PARTIAL_ARTIFACTS:
        (meeting_dir / name).unlink(missing_ok=True)


def best_effort_cleanup_finalize_artifacts(meeting_dir: Path) -> None:
    """Remove every Phase A + Phase B sidecar file from ``meeting_dir``.

    Used by the cleanup-only recovery path when a COMPLETE meeting has
    leftover finalize artifacts (Phase B crashed after the COMPLETE
    transition but before its end-of-step-7 cleanup). Idempotent.
    """
    for name in _PHASE_B_ARTIFACTS:
        (meeting_dir / name).unlink(missing_ok=True)
...
class MeetingStorage:
    ...
    def startup(self) -> RecoverInterruptedResult:
        """Initialize storage: recover interrupted meetings and run cleanup.

        Call once at server start before accepting any connections. Returns
        the recovery triage so the lifespan-level driver can spawn Phase B
        for ``finalizing_ids`` and run cleanup-only sweeps for ``cleanup_ids``.
        """
        self._meetings_dir.mkdir(parents=True, exist_ok=True)
        result = self.recover_interrupted()
        if result.interrupted_count:
            logger.info("Recovered %d interrupted meeting(s)", result.interrupted_count)
        if result.finalizing_ids:
            logger.info(
                "%d meeting(s) require Phase B resume: %s",
                len(result.finalizing_ids),
                ", ".join(result.finalizing_ids),
            )
        if result.cleanup_ids:
            logger.info(
                "%d meeting(s) require finalize-leftover cleanup: %s",
                len(result.cleanup_ids),
                ", ".join(result.cleanup_ids),
            )
    ...
    def recover_interrupted(self) -> RecoverInterruptedResult:
        """Triage every meeting on disk for crash recovery.

        Per the Phase A / Phase B split's recovery contract, the
        authoritative marker for "this meeting is mid-finalize" is the
        durable ``finalize_context.json`` file (with ``phase_a_done:
        true``), not ``meta.state`` — the meta transition can race the
        durable write in either direction. The mapping:

        =========================  ================  ================  ===========================================
        meta.state                 finalize_context  phase_a_done      Action
        =========================  ================  ================  ===========================================
        RECORDING                  absent            —                 → INTERRUPTED
        RECORDING                  present           false             → INTERRUPTED + sweep partial sidecars
        RECORDING                  present           true              → force FINALIZING, schedule Phase B
        FINALIZING                 present           true              → schedule Phase B
        FINALIZING                 present           false             → INTERRUPTED
        FINALIZING                 absent            —                 → INTERRUPTED
        FINALIZING (mtime > TTL)   any               —                 → INTERRUPTED (defensive)
        COMPLETE                   present           —                 → schedule cleanup-only
        =========================  ================  ================  ===========================================
        ...
        Returns:
            ``RecoverInterruptedResult`` listing the per-bucket meeting
            ids that the lifespan recovery driver should act on. The
            ``interrupted_count`` is the number marked INTERRUPTED in
            this pass; ``finalizing_ids`` is the list to spawn Phase B
            for; ``cleanup_ids`` is the list with leftover finalize
            artifacts to sweep.
        """
        result = RecoverInterruptedResult(interrupted_count=0, finalizing_ids=[], cleanup_ids=[])
        if not self._meetings_dir.exists():
            return result

        # TTL for stale FINALIZING — anything older than this is no
        # longer worth resuming and falls through to INTERRUPTED so the
        # restart loop can't spin on a permanently-failing meeting.
        max_age_s = float(os.environ.get("SCRIBE_FINALIZE_MAX_AGE_S", "3600"))
        ...
        for meeting_dir in self._meetings_dir.iterdir():
            ...
            ctx_present = ctx_path.exists()
            phase_a_done = False
            if ctx_present:
                try:
                    payload = json.loads(ctx_path.read_text())
                    phase_a_done = bool(payload.get("phase_a_done", False))
                except Exception:
                    logger.warning(
                        "Corrupt finalize_context.json in %s — treating as missing",
                        meeting_dir.name,
                    )
                    ctx_present = False

            # Stale-FINALIZING TTL: if mtime of meta exceeds TTL, downgrade.
            if meta.state == MeetingState.FINALIZING:
                age_s = time.time() - meta_path.stat().st_mtime
                if age_s > max_age_s:
                    logger.warning(
                        "Meeting %s in FINALIZING for %.0fs > %.0fs TTL — "
                        "marking INTERRUPTED and sweeping leftovers",
                        meta.meeting_id,
                        age_s,
                        max_age_s,
                    )
                    _sweep_phase_a_artifacts(meeting_dir)
                    meta.state = MeetingState.INTERRUPTED
                    self._write_meta(meta)
                    result = result._replace(interrupted_count=result.interrupted_count + 1)
                    continue

            # RECORDING with finalize_context.json present and phase_a_done
            # → force-transition to FINALIZING and schedule Phase B
            # resume. (Crash between Phase A's marker rewrite and the
            # transition_state call.)
            if meta.state == MeetingState.RECORDING and ctx_present and phase_a_done:
                self.transition_state(meta.meeting_id, MeetingState.FINALIZING)
                result = result._replace(
                    finalizing_ids=[*result.finalizing_ids, meta.meeting_id],
                )
                logger.info(
                    "Meeting %s: RECORDING + finalize_context (phase_a_done) "
                    "— force-transitioned to FINALIZING for Phase B resume",
                    meta.meeting_id,
                )
                continue

            # FINALIZING with phase_a_done=true → resume Phase B normally.
            if meta.state == MeetingState.FINALIZING and ctx_present and phase_a_done:
                result = result._replace(
                    finalizing_ids=[*result.finalizing_ids, meta.meeting_id],
                )
                logger.info(
                    "Meeting %s: FINALIZING + phase_a_done — scheduling Phase B resume",
                    meta.meeting_id,
                )
                continue

            # COMPLETE with leftover finalize_context.json → cleanup-only
            # (Phase B finished but crashed before _cleanup_finalize_context_files).
            if meta.state == MeetingState.COMPLETE and ctx_present:
                result = result._replace(
                    cleanup_ids=[*result.cleanup_ids, meta.meeting_id],
                )
                logger.info(
                    "Meeting %s: COMPLETE with leftover finalize_context — "
                    "scheduling cleanup-only pass",
                    meta.meeting_id,
                )
                # Don't `continue` — fall through to empty-meeting GC below
                # so a COMPLETE-with-leftovers meeting can still be GC'd
                # if its journal is empty.
                # (Note: cleanup_ids will still get the leftover sweep in
                # the recovery driver. Empty-meeting GC may delete the
                # meeting entirely, in which case the driver's cleanup
                # is a no-op via missing_ok unlinks.)

            # RECORDING + (no ctx OR phase_a_done=false) → INTERRUPTED.
            # FINALIZING + (no ctx OR phase_a_done=false) → INTERRUPTED.
            elif meta.state in (MeetingState.RECORDING, MeetingState.FINALIZING):
                if ctx_present and not phase_a_done:
                    # Phase A crashed mid-write — sweep its partial sidecars
                    # so the meeting_dir is in a known-bad state.
                    _sweep_phase_a_artifacts(meeting_dir)
                meta.state = MeetingState.INTERRUPTED
                self._write_meta(meta)
                result = result._replace(interrupted_count=result.interrupted_count + 1)
    ...
    # ------------------------------------------------------------------
    # Phase B progress sidecar
    # ------------------------------------------------------------------
    #
    # A meeting that has been Stopped but is still finalizing in the
    # background needs to be visible to the operator across page reloads
    # and detail-view navigation. The legacy sync finalize emitted a
    # ``finalize_progress`` WS event with per-step ETA; the async split
    # broadcast events without persisting them, so a reload showed
    # nothing while Phase B was still grinding.
    #
    # The sidecar lives at ``<meeting_dir>/phase_b_progress.json`` and
    # has three possible shapes, distinguished by ``terminal`` + ``error``:
    #
    #   * in-flight (terminal=false): step / total / label / eta_seconds /
    #     session_uuid / started_at. ``session_uuid`` stamps the writing
    #     process so the boot sweep can tell live state from a crashed
    #     prior process.
    #   * success-terminal: never persisted (cleared instead).
    #   * failure-terminal (terminal=true, error set): step / total /
    #     label / error.{code, message, step}. Persists until reprocess
    #     succeeds or operator dismisses.

    def _phase_b_sidecar_path(self, meeting_id: str) -> Path:
        return self._meeting_dir(meeting_id) / "phase_b_progress.json"

    def _write_phase_b_atomic(self, meeting_id: str, payload: dict) -> None:
        path = self._phase_b_sidecar_path(meeting_id)
        path.parent.mkdir(parents=True, exist_ok=True)
        tmp = path.with_suffix(path.suffix + f".tmp.{os.getpid()}")
        tmp.write_text(json.dumps(payload, indent=2) + "\n")
        tmp.replace(path)

    def write_phase_b_progress(
        self,
        meeting_id: str,
        *,
        step: int,
        ...
        """Persist an in-flight Phase B progress tick.

        Stamped with the current process's ``session_uuid`` so the boot
        sweep can distinguish "this process is still working" from
        "previous process crashed mid-step".
        """
        self._write_phase_b_atomic(
            meeting_id,
            {
                "terminal": False,
                "step": int(step),
    ...
    def write_phase_b_failure(
        ...
        *,
        ...
        """Persist a Phase B failure-terminal sidecar.

        ``code`` is a stable string consumed by the UI for branching
        (e.g. ``diarize_failed``, ``summary_failed``, ``interrupted``,
        ``phase_b_failed``). ``message`` is operator-safe text — raw
        exception text is never persisted; it goes to the log instead.
        Sidecar lives until reprocess succeeds or operator dismisses.
        """
        self._write_phase_b_atomic(
            meeting_id,
            {
    ...
    def clear_phase_b_progress(self, meeting_id: str) -> None:
        """Remove the sidecar — called on success-terminal and reprocess success."""
        self._phase_b_sidecar_path(meeting_id).unlink(missing_ok=True)

    def read_phase_b_progress(self, meeting_id: str) -> dict | None:
        """Return the sidecar verbatim, or None if absent / unreadable."""
        path = self._phase_b_sidecar_path(meeting_id)
        if not path.exists():
            return None
        try:
    ...
    def sweep_orphan_phase_b_sidecars(self, current_session_uuid: str) -> list[str]:
        """Rewrite stale in-flight sidecars from prior processes.

        Called once at lifespan boot, before any client can read meeting
        metadata. Any sidecar that is ``terminal=false`` AND whose
        ``session_uuid`` doesn't match the current process is by
        definition an orphan from a crashed prior boot — Phase B tasks
        are in-memory and don't survive a process restart. Rewrite it
        as a failure-terminal sidecar with ``code="interrupted"`` so the
        API never returns ambiguous in-flight state.

        Returns the list of meeting IDs that were rewritten.
        """
        rewritten: list[str] = []
        if not self._meetings_dir.exists():
            return rewritten
        for meeting_dir in self._meetings_dir.iterdir():
            if not meeting_dir.is_dir():
                continue
            sidecar = meeting_dir / "phase_b_progress.json"
            ...
            if data.get("terminal") is True:
                continue
            if data.get("session_uuid") == current_session_uuid:
                continue
            mid = meeting_dir.name
            self.write_phase_b_failure(
                mid,
                step=int(data.get("step", 0) or 0),
                total=int(data.get("total_steps", 7) or 7),
                label=str(data.get("label", "") or ""),
                code="interrupted",
                message="Finalize was interrupted — try Reprocess",
            )
            rewritten.append(mid)
        return rewritten
