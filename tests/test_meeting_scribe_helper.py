...
from meeting_scribe.helper_client import HelperError
from meeting_scribe.helper_client import call as client_call
...
def test_caller_authorized_root_only_rejects_service_uid() -> None:
    ...
    spec = VerbSpec(handler=verb_regdomain_set, allowed_uids="root_only")
...
def test_caller_authorized_root_and_service() -> None:
    spec = VerbSpec(handler=verb_regdomain_set, allowed_uids="root_and_service")
...
async def helper_socket(tmp_path: Path) -> Any:
    ...
    server_task = asyncio.create_task(serve(socket_path=sock_path, service_uid=service_uid))
...
def test_verb_registry_mutating_verbs_root_only() -> None:
    ...
    UID."""
    mutating = {"wifi.up", "wifi.down", "regdomain.set"}
