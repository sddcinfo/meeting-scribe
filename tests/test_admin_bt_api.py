...
def test_pair_surfaces_bluetooth_error() -> None:
    ...
    surfaces a 502 with a stable ``bt_backend_failed`` code +
    ``operation: "pair"`` field so the UI can show the CLI
    fallback. The exception detail goes to the server log only —
    CodeQL flagged returning ``str(exc)`` as info exposure
    (the str includes filesystem paths + bluetoothctl-internal
    state)."""
    ...
    body = resp.json()
    assert body["error"] == "bt_backend_failed"
    assert body["operation"] == "pair"
...
def test_mic_persists_setting(tmp_path, monkeypatch) -> None:
    ...
    with (
        patch(
            "meeting_scribe.bt.apply_bt_input_state",
            new=AsyncMock(return_value={"updated": [], "skipped": []}),
        ),
        TestClient(app, base_url="http://test") as client,
    ):
