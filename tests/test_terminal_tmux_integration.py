...
_TEST_CRED = "TmuxIntegrationPassword42"
...
async def test_resize_window_no_server_returns_false():
    """resize_window returns False when there's no server running on
    the socket — defensive: pty_session.resize() shouldn't crash if the
    tmux server died between spawn and the next resize."""
    ok = await tmux_helper.resize_window("no-such-session", 80, 24, socket="scribe-test-no-server")
    assert ok is False


async def test_resize_window_succeeds_on_live_session(test_config):
    """End-to-end resize-window: spawn a session, resize it, query
    the window dimensions back from tmux to confirm the change took.
    Belt-and-suspenders for the per-tab tmux session contract — even
    though tmux's default `latest` policy already follows TIOCSWINSZ
    on a single-client session, calling resize-window explicitly makes
    sizing deterministic if a second client is ever attached manually
    (e.g. by an operator using `tmux -L scribe attach -t scribe-X`)."""
    argv = tmux_helper.build_argv("rsz", config=test_config, socket=TEST_SOCKET)
    argv_detached = (
        argv[: argv.index("new-session") + 1] + ["-d"] + argv[argv.index("new-session") + 1 :]
    )
    _sh(argv_detached)
    try:
        ok = await tmux_helper.resize_window("rsz", 100, 30, socket=TEST_SOCKET)
        assert ok is True
        out = _sh(
            [
                "tmux",
...
def app_factory(tmp_path, monkeypatch, test_config):
    ...
    # v1.0 admin auth is cookie-only — TestClient just needs the
    # ``admin-password-hmac`` for ``_TEST_CRED`` (written below)
    # plus a per-test SCRIBE_STATE_DIR to point setup_state at.
    ...
    def make() -> tuple[FastAPI, ActiveTerminals]:
        ...
        expected = _hmac.new(admin_secret.secret, _TEST_CRED.encode("utf-8"), "sha256").hexdigest()
...
def _attach(client: TestClient, app) -> tuple[Any, str]:
    ...
    r = client.post("/api/admin/authorize", json={"password": _TEST_CRED})
