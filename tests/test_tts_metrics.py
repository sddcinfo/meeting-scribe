...
class TestLoopLagMonitorRecords:
    ...
    @pytest.mark.asyncio
    async def test_monitor_records_at_least_one_sample(self, monkeypatch):
        from meeting_scribe.runtime import health_monitors
        from meeting_scribe.runtime.health_monitors import loop_lag_monitor

        # Shrink the production 0.5 s tick interval so two ticks land
        # in tens of milliseconds instead of holding up the suite for
        # 1.2 s on every run.
        monkeypatch.setattr(health_monitors, "_LOOP_LAG_TICK_S", 0.02)

        task = asyncio.create_task(loop_lag_monitor())
        try:
            await asyncio.sleep(0.1)  # ≥ 2 ticks at the test interval
        finally:
            task.cancel()
            try:
                await task
...
class TestSilenceWatchdogStaleTimestampGuard:
    ...
    async def test_does_not_fire_on_stale_pre_meeting_timestamp(self, monkeypatch):
        ...
        # Shrink the production 1-second poll to milliseconds so the
        # test can confirm two ticks without a real-time wait.
        monkeypatch.setattr(health_monitors, "SILENCE_WATCHDOG_POLL_INTERVAL_S", 0.05)
        ...
        try:
            ...
            await asyncio.sleep(0.2)  # ≥ 2 polls at the test interval
        finally:
            task.cancel()
            try:
                await task
