...
_TEST_CRED = "TestLogoutPassword4242"
...
def _build_app(tmp_path, monkeypatch=None) -> tuple[FastAPI, BootstrapConfig]:
    secret_store = AdminSecretStore.load_or_create(tmp_path / "admin-secret")
    state_dir = tmp_path / "scribe-state"
    state_dir.mkdir(parents=True, exist_ok=True)
    # Persist the wizard ``admin-password-hmac`` for ``_TEST_CRED``
    # so ``setup_state.verify_admin_password`` accepts it. v1.0
    # admin auth is cookie-only — no AP-subnet env knob needed.
    ...
    if monkeypatch is not None:
        # ``has_admin_session`` reads the global signer; install ours.
...
def test_logout_endpoint_revokes_session(tmp_path, monkeypatch) -> None:
    app, cfg = _build_app(tmp_path, monkeypatch)
...
def test_logout_idempotent_without_cookie(tmp_path, monkeypatch) -> None:
...
def test_logout_idempotent_with_garbage_cookie(tmp_path, monkeypatch) -> None:
    app, _ = _build_app(tmp_path, monkeypatch)
    with TestClient(app, base_url="https://test") as client:
...
def test_deauthorize_alias_invokes_same_path(tmp_path, monkeypatch) -> None:
    """/api/admin/deauthorize is a deprecated alias for /api/admin/logout
    — same revocation behavior."""
    app, cfg = _build_app(tmp_path, monkeypatch)
    with TestClient(app, base_url="https://test") as client:
        client.post("/api/admin/authorize", json={"password": _TEST_CRED})
        cookie_value = client.cookies.get(COOKIE_NAME)
        ok, sid, _ = decode_verified_cookie(cfg.cookie_signer, cookie_value)
...
def test_reauth_revokes_prior_session(tmp_path, monkeypatch) -> None:
    ...
    with TestClient(app, base_url="https://test") as client:
        # First auth.
        r1 = client.post("/api/admin/authorize", json={"password": _TEST_CRED})
        ...
        r2 = client.post("/api/admin/authorize", json={"password": _TEST_CRED})
        assert r2.status_code == 200
        second_cookie = client.cookies.get(COOKIE_NAME)
        _, second_sid, _ = decode_verified_cookie(cfg.cookie_signer, second_cookie)
...
def test_authorize_uses_strict_samesite(tmp_path, monkeypatch) -> None:
