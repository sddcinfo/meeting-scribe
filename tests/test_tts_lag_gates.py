...
class TestTimeoutRecoveryFlush:
    """A wedged synthesis should clear stale queued audio work."""

    def test_flush_drops_every_queued_item_and_counts_it(self):
        runtime_state.tts_queue = asyncio.Queue(maxsize=4)
        now = time.monotonic()
        for _ in range(3):
            runtime_state.tts_queue.put_nowait((_make_event("hello"), "default", now + 4.0, now))

        dropped = _flush_queued_tts_after_timeout()

        assert dropped == 3
        assert runtime_state.tts_queue.qsize() == 0
        assert runtime_state.metrics.tts_dropped_recovery_flush == 3


class TestProducerQueueCapacity:
    """The producer queue should absorb realistic TTS bursts."""

    def test_admits_burst_without_drop_when_queue_has_capacity(self):
        runtime_state.tts_queue = asyncio.Queue(maxsize=128)
        now = time.monotonic()

        for idx in range(32):
            evt = _make_event(
                f"Important translated sentence number {idx}.",
                utterance_end_at=now,
                completed_at=now,
            ).model_copy(update={"segment_id": f"seg-{idx}"})
            _enqueue_tts(evt, "default")

        assert runtime_state.tts_queue.qsize() == 32
        assert runtime_state.metrics.tts_submitted == 32
        assert runtime_state.metrics.tts_dropped_queue_full == 0
...
class TestProducerStaleGate:
    ...
    def test_drops_stale_when_deadline_already_blown(self):
        ...
        evt = _make_event("Hello", utterance_end_at=time.monotonic() - 20.0)
        _enqueue_tts(evt, "default")
