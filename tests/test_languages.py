...
class TestLanguageRegistry:
    ...
    def test_core_studio_voices_are_native_speakers(self):
        from meeting_scribe.backends.tts_voices import studio_voice_for

        # English moved from "ryan" to "aiden" on 2026-05-13 after a
        # live audit — Aiden reads as clearer American English on
        # short button-feedback strings. See backends/tts_voices.py.
        assert studio_voice_for("en") == "aiden"
        assert studio_voice_for("EN") == "aiden"
        assert studio_voice_for("ja") == "ono_anna"
        assert studio_voice_for("JA") == "ono_anna"
...
class TestTranslationPromptPriorContext:
    ...
    Used by the live JA→EN path when
    ``live_translate_context_window_ja_en`` > 0 to anchor the LLM on
    the meeting's ongoing topic / speakers / proper-nouns so fragmented
    utterances don't get hallucinated into free-standing sentences.
    Default 0 leaves prior_context=None to keep the latency SLO
    unaffected.
    """
    ...
    def test_empty_context_is_noop(self):
        # An empty list is also a no-op — keeps the "no prior
        # translations yet" path from injecting an empty context block.
        cold = get_translation_prompt("ja", "en")
