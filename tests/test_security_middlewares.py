...
def test_csp_includes_required_directives() -> None:
    ...
    ``require-trusted-types-for 'script'`` was tried earlier but
    broke every dynamic UI render (innerHTML without a registered
    Trusted Types policy). Restore once a TT migration lands; until
    then keep its absence locked in to prevent re-introduction
    without the matching JS work."""
    for required in (
        "default-src 'self'",
        "script-src 'self'",
        "media-src 'self' blob:",
    ...
    assert "require-trusted-types-for" not in _CSP_HEADER_VALUE, (
        "Trusted Types directive re-introduced without a JS policy "
        "migration — see middlewares.py module docstring"
    )
...
def test_cache_no_store_on_auth_page() -> None:
    """``/auth`` is the post-setup sign-in surface; it must not be
    cached so the back-button after logout doesn't show a stale
    authenticated view."""
    app = _build_app()
    with TestClient(app, base_url="http://test") as client:
