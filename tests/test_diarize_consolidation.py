...
@pytest.mark.asyncio
async def test_degraded_backend_backs_off_before_retry_window():
    class ExplodingClient:
        async def post(self, *args, **kwargs):  # pragma: no cover - should never be called
            raise AssertionError("diarize backend should not be called during cooldown")
    ...
    be._next_retry_ts = _time.monotonic() + 60.0

    audio = np.zeros(8 * 16000, dtype=np.float32)
    result = await be.process_audio(audio, sample_offset=0)
