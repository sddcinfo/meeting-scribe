...
@pytest.fixture(autouse=True)
def _no_driver_settle_sleep(monkeypatch: pytest.MonkeyPatch) -> None:
    """Neutralize the production wifi-driver settle sleeps in BOTH
    the rotation path (``ap_control._rotate_profile_and_bounce``,
    2 s) and the bring-up path (``wifi._bring_up_ap``, 1 s) so the
    test suite runs in milliseconds instead of stacking ``2–3 s ×
    N tests = 20+`` seconds of dead wall-clock time."""
    monkeypatch.setattr(_ap_control, "_AP_BOUNCE_DRIVER_SETTLE_S", 0.0)
    monkeypatch.setattr(wifi, "_AP_BOUNCE_DRIVER_SETTLE_S", 0.0)
...
# ---------------------------------------------------------------------------
# Phase 3+4 — AP rotation self-heal + split TTL + rotation state surface
# ---------------------------------------------------------------------------


class TestAPRotationSelfHeal:
    """When ``nmcli con modify`` rejects with 'key-mgmt: property is
    missing', the rotation path must delete the connection and re-add
    it from scratch with WPA3-SAE args, then retry the bounce. The
    2026-05-14 demo failure had been silently logging the WARN forever
    because the bug was real but the recovery path didn't exist."""

    @patch("meeting_scribe.wifi.subprocess.run")
    def test_recreate_helper_issues_delete_then_add_sequence(self, mock_run: MagicMock) -> None:
        """The recreate helper deletes the stale profile (so the
        inconsistent wifi-sec state is gone) and re-adds it from
        scratch with the SAE security args."""
        mock_run.return_value = _mk_completed()
        # Avoid the netplan-scrub I/O — it tries to glob /etc/netplan.
        with patch("meeting_scribe.wifi._scrub_netplan_ap_yaml"):
            wifi._recreate_ap_profile_with_sae("Demo-AP-7B6E", "abc-12345")
        commands = [call.args[0] for call in mock_run.call_args_list]
        # First subprocess call is the delete; second is the add.
        assert commands[0] == ["sudo", "nmcli", "con", "delete", "DellDemo-AP"]
        # The add must include SAE key-mgmt + the new credentials.
        add_argv = commands[1]
        assert add_argv[:4] == ["sudo", "nmcli", "con", "add"]
        assert "sae" in add_argv
        assert "abc-12345" in add_argv
        assert "Demo-AP-7B6E" in add_argv


class TestRotationStateSurface:
    """Phase 3.3 — ``/api/status`` consumers must see the most recent
    rotation outcome (target vs live SSID + timestamp)."""

    def test_get_last_rotation_state_defaults_to_rotation_ok_true(self) -> None:
        _ap_control._reset_rotation_state_for_tests()
        # Reset the module-level rotation state explicitly.
        _ap_control._LAST_ROTATION_TARGET_SSID = None  # type: ignore[assignment]
        _ap_control._LAST_ROTATION_LIVE_SSID = None  # type: ignore[assignment]
        _ap_control._LAST_ROTATION_AT = None  # type: ignore[assignment]
        _ap_control._LAST_ROTATION_OK = True  # type: ignore[assignment]
        snap = _ap_control.get_last_rotation_state()
        assert snap["rotation_ok"] is True
        assert snap["target_ssid"] is None
        assert snap["live_ssid"] is None
        assert snap["last_rotation_at"] is None

    def test_record_rotation_outcome_match_sets_ok_true(self) -> None:
        _ap_control._record_rotation_outcome(target="Dell-AP-1", live="Dell-AP-1", at=1234.5)
        snap = _ap_control.get_last_rotation_state()
        assert snap["rotation_ok"] is True
        assert snap["target_ssid"] == "Dell-AP-1"
        assert snap["live_ssid"] == "Dell-AP-1"
        assert snap["last_rotation_at"] == 1234.5

    def test_record_rotation_outcome_mismatch_sets_ok_false(self) -> None:
        _ap_control._record_rotation_outcome(target="Dell-AP-NEW", live="Dell-AP-OLD", at=2000.0)
        snap = _ap_control.get_last_rotation_state()
        assert snap["rotation_ok"] is False
        assert snap["target_ssid"] == "Dell-AP-NEW"
        assert snap["live_ssid"] == "Dell-AP-OLD"

    def test_record_rotation_outcome_live_none_sets_ok_false(self) -> None:
        _ap_control._record_rotation_outcome(target="Dell-AP", live=None, at=3000.0)
        snap = _ap_control.get_last_rotation_state()
        assert snap["rotation_ok"] is False


class TestSplitTTLContract:
    """Phase 4 — ``_nmcli_ap_is_active`` (and
    ``_nmcli_read_live_ap_credentials``) accept a per-call ``ttl`` and
    ``bypass_cache`` so the rotation activation loop keeps observing
    transitions in real time while the high-volume status panel reads
    the cached value."""

    @patch("meeting_scribe.wifi.subprocess.run")
    def test_default_ttl_matches_legacy_5s_window(self, mock_run: MagicMock) -> None:
        """No-arg calls keep the original behavior (no breaking change)."""
        mock_run.return_value = _mk_completed(stdout="DellDemo-AP:wlP9s9\n")
        for _ in range(3):
            assert wifi._nmcli_ap_is_active() is True
        # All three within 5s — only one subprocess call.
        assert mock_run.call_count == 1

    @patch("meeting_scribe.wifi.subprocess.run")
    def test_long_ttl_amortizes_status_panel_polls(self, mock_run: MagicMock) -> None:
        """``/api/status`` (and similar high-volume sites) pass the
        long TTL so 30 reads in 25 s issue 1 subprocess call."""
        mock_run.return_value = _mk_completed(stdout="DellDemo-AP:wlP9s9\n")
        # Multiple reads with the long TTL — all should hit cache.
        for _ in range(10):
            wifi._nmcli_ap_is_active(ttl=wifi._AP_STATE_STATUS_TTL_S)
        assert mock_run.call_count == 1

    @patch("meeting_scribe.wifi.subprocess.run")
    def test_bypass_cache_forces_fresh_subprocess_read(self, mock_run: MagicMock) -> None:
        """Post-condition reads after a rotation must bypass the TTL
        cache because the cache could otherwise serve a value from
        before the bounce."""
        mock_run.return_value = _mk_completed(stdout="DellDemo-AP:wlP9s9\n")
        wifi._nmcli_ap_is_active()  # primes the cache
        assert mock_run.call_count == 1
        wifi._nmcli_ap_is_active(bypass_cache=True)
        assert mock_run.call_count == 2
        # Same for the credentials reader.
        wifi._invalidate_ap_state_cache()
        mock_run.reset_mock()
        mock_run.return_value = _mk_completed(stdout="802-11-wireless.ssid:Foo\n")
        wifi._nmcli_read_live_ap_credentials(bypass_cache=True)
        wifi._nmcli_read_live_ap_credentials(bypass_cache=True)
        # Two bypass reads → two subprocess calls (no caching).
        assert mock_run.call_count == 2
