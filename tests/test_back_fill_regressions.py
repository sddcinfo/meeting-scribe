...
SCRIBE_JS_ROOT = REPO_ROOT / "static" / "js" / "scribe"
INDEX_HTML = REPO_ROOT / "static" / "index.html"


def _scribe_app_source() -> str:
    """Concatenate every scribe-app JS source the static greps below
    care about.

    Phase 3 of the Tailwind migration extracted code out of the
    original ``scribe-app.js`` (briefly renamed ``_legacy.js``, now
    finally living at ``features/admin-boot.js``) into the
    ``features/*.js`` tree. The regression tests in this file are
    *content-level* — they grep for inline-markup snippets (e.g.
    ``id="sv-trans-status"``) or DOM-binding shapes that may sit in
    any feature module. Concatenating every feature source keeps the
    assertions valid across the whole migration surface.
    """
    parts = []
    # _legacy.js was renamed to features/admin-boot.js once the boot
    # extraction landed; keep the optional read of the historical name
    # so this test file works against either tree state.
    legacy = SCRIBE_JS_ROOT / "_legacy.js"
    if legacy.exists():
        parts.append(legacy.read_text())
    for path in sorted((SCRIBE_JS_ROOT / "features").glob("*.js")):
