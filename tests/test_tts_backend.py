...
from meeting_scribe.tts.worker import (
    _is_tts_translator_meta_response,
    _text_matches_tts_target_language,
)
...
class TestLanguageGate:
    ...
    def test_tts_target_language_rejects_japanese_text_for_english(self):
        assert not _text_matches_tts_target_language("結果が出ました。", "en")

    def test_tts_target_language_rejects_english_text_for_japanese(self):
        assert not _text_matches_tts_target_language("The result is ready.", "ja")

    def test_tts_target_language_allows_expected_en_ja_scripts(self):
        assert _text_matches_tts_target_language("The result is ready.", "en")
        assert _text_matches_tts_target_language("結果が出ました。", "ja")
...
class TestFramedStream:
    """Container /stream endpoint frame parsing."""

    @pytest.mark.asyncio
    async def test_iter_framed_pcm_handles_split_frames(self):
        samples = np.array([1, -2, 300], dtype=np.int16)
        payload = samples.tobytes()
        wire = struct.pack(">I", len(payload)) + payload + struct.pack(">I", 0)
        ...
        chunks = [chunk async for chunk in Qwen3TTSBackend._iter_framed_pcm(FakeResponse())]

        assert len(chunks) == 1
        np.testing.assert_array_equal(chunks[0], samples)


class TestTranslatorMetaGate:
    """Translator refusal/meta text should not be spoken by TTS."""

    def test_drops_apology_response(self):
        assert _is_tts_translator_meta_response(
            'I\'m sorry, but the input "プリプリ" does not appear to be meaningful text.'
        )

    def test_drops_cannot_translate_response(self):
        assert _is_tts_translator_meta_response("I cannot translate that as provided.")

    def test_allows_real_translation(self):
        assert not _is_tts_translator_meta_response("Please move the meeting to tomorrow morning.")
