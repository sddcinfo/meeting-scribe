...
class TestWatchdogCounter:
    ...
    @pytest.mark.asyncio
    async def test_sub_vad_watchdog_flush_does_not_count_or_escalate(self, monkeypatch):
        """Room noise/silence must not drive watchdog recovery.

        The incident pattern was repeated "empty in-flight submissions deque"
        escalations: watchdog accounting ran before VAD, so sub-threshold audio
        incremented recovery counters without ever submitting an ASR request.
        """
        m = Metrics()
        monkeypatch.setattr(runtime_state, "metrics", m, raising=False)
        b = VllmASRBackend(buffer_seconds=3.5)
        b._client = SimpleNamespace(post=AsyncMock())
        b._model = "test-model"
        b._last_response_time = time.monotonic() - 20.0
        b._watchdog_escalation_threshold = 1

        silence = np.zeros(SAMPLE_RATE, dtype=np.int16).tobytes()
        await b.process_audio_bytes(silence, sample_offset=0)

        assert b._recovery_state == "NORMAL"
        assert b._watchdog_consecutive_fires == 0
        assert m.watchdog_fires_total == 0
        assert m.watchdog_escalations_total == 0
        b._client.post.assert_not_awaited()
...
async def test_probe_fail_then_success_with_auto_recreate(wired_state, monkeypatch):
    ...
    # ``compose_restart`` is invoked through ``asyncio.to_thread``,
    # which expects a SYNCHRONOUS callable — patching it with
    # AsyncMock leaves the resulting coroutine unawaited and
    # produces a RuntimeWarning at GC time. MagicMock matches the
    # real callable's sync signature and the assert_called_once_with
    # below works the same.
    from unittest.mock import MagicMock

    fake_compose = MagicMock(return_value=None)
