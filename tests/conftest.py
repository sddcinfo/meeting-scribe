...
def _scribe_state_dir(tmp_path_factory, monkeypatch):
    """Default ``SCRIBE_STATE_DIR`` to a per-test tmp dir AND write the
    ``setup-complete`` marker so the setup-mode redirect in
    ``meeting_scribe.middlewares`` short-circuits.

    Tests that exercise setup mode (``tests/test_setup_state.py``)
    override SCRIBE_STATE_DIR with their own tmp_path which doesn't
    have the marker, so they see the unconfigured state. Tests that
    use ``server.app`` directly (TestClient against the production
    routes) get the operating-mode short-circuit by default.
    """
    state_dir = tmp_path_factory.mktemp("scribe-state")
    monkeypatch.setenv("SCRIBE_STATE_DIR", str(state_dir))
    (state_dir / "setup-complete").write_text("1\n")
    # Tests don't drive the auth-gate flow; bypass the hotspot
    # guard so plain-http TestClient calls don't 302 to /auth.
    monkeypatch.setenv("SCRIBE_HOTSPOT_GUARD_BYPASS", "1")
