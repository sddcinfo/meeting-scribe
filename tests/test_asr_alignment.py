...
class TestUtteranceEndClockRepair:
    async def test_active_meeting_repairs_missing_audio_wall_at_start(self, backend, monkeypatch):
        from meeting_scribe.runtime import state

        be, captured = backend
        be.audio_wall_at_start = None
        monkeypatch.setattr(
            state,
            "current_meeting",
            SimpleNamespace(meeting_id="m1"),
            raising=False,
        )
        monkeypatch.setattr(
            state,
            "metrics",
            SimpleNamespace(meeting_start=1234.0),
            raising=False,
        )
        ...
        assert captured
        assert be.audio_wall_at_start == 1234.0
        assert captured[0].utterance_end_at == 1236.0
