...
class TestLowValueEnglishFragments:
    """Tiny English fragments are usually harmful in ja/en live ASR."""

    def test_filters_ultra_short_fragments(self):
        assert _is_low_value_english_fragment("I'm.")
        assert _is_low_value_english_fragment("the.")
        assert _is_low_value_english_fragment("Okay.")

    def test_keeps_complete_english_turns(self):
        assert not _is_low_value_english_fragment("The selection results are ready.")
        assert not _is_low_value_english_fragment("Yeah, it's out there.")
