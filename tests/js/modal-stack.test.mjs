// Regression tests for the modal stack.
//
// Run:  node --test tests/js/
//
// Source under test: static/js/scribe/features/modal-system.js (the
// pure ES module extracted from scribe-app.js in Phase 3 Tier 2).
// The rules enforced here (also documented in docs/styling.md):
//
...
//
// We import only what we need by reading the module source, slicing
// out the modal core (showModal..closeAllModals), stripping the
// ``export `` keyword + the ``../lib/escape.js`` import, and
// evaluating against a tiny DOM shim. Dialog primitives
// (alertDialog/confirmDialog/promptDialog) live further down the
// module and are not tested here.
...
const SRC = readFileSync(
  resolve(here, '../../static/js/scribe/features/modal-system.js'),
  'utf8',
);

// Extract the modal core (showModal, closeModal, _modalEscHandler,
// closeAllModals). The slice runs from ``export function showModal``
// to the line just before the first dialog primitive
// (``export async function confirmDialog``). Then ``export `` is
// stripped so the slice is plain function declarations the
// ``new Function`` closure factory can eval.
function sliceModalCore(src) {
  const start = src.indexOf('export function showModal');
  const end = src.indexOf('export async function confirmDialog');
  assert.ok(start !== -1, 'showModal export marker not found in modal-system.js');
  assert.ok(end !== -1, 'confirmDialog export marker not found in modal-system.js');
  return src.slice(start, end).replace(/\bexport\s+/g, '');
}
...
test('showModal opens on the root card and activates it', () => {
  ...
  // Overlay defaults to display:none in CSS; showModal opens it explicitly to
  // 'flex' (was '' in pre-v1.0 builds when the rule defaulted to display:flex).
