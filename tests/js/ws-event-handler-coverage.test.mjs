...
function popoutHandlerName(msg) {
  ...
  // meeting_started: kiosk popout flips layout to 'translate' preset and
  // sets body[data-recording] — distinct branch from the stop/cancel
  // reset above. Lives at _legacy.js:2084 in the popout cascade.
  if (msg.type === 'meeting_started') return '_lifecycle_meeting_started';
  switch (msg.type) {
    ...
    case 'interpretation_status': return 'noop_interpretation_status';
    case 'bt_status':            return 'noop_bt_status';
    ...
    case 'background_finalize_progress': return 'background_finalize_progress_toast';
    case 'slide_change':
    case 'slide_deck_changed':
    case 'slide_partial_ready':
    case 'slide_job_progress':   return '_handleSlideWsEvent';
    case 'popout_layout_changed': return 'noop_popout_layout';
  }
...
function adminHandlerName(msg) {
  ...
  if (msg.type === 'interpretation_status') return 'interpretation_status';
  if (msg.type === 'bt_status') return 'bt_status';
  if (msg.type === 'mic_level') return 'mic_level';
...
const ADMIN_OPTIONAL = new Set([
  ...
  'audio_drift',
  'finalize_progress',
  // Phase B's progress channel is consumed by the corner toast in
  // both popout and admin views; the popout cascade has the named
  // handler and the admin tab listens to it via the toast manager,
  // so the strict registry test is the authoritative gate.
  'background_finalize_progress',
  'transcript_revision',
  // Pop-out layout broadcasts are consumed by the pop-out window only
  // (the admin tab opens the popouts; it does not need to react to
  // its own layout-change echoes). Strict registry test still gates
  // the existence of a sample fixture for the slug.
  'popout_layout_changed',
  // meeting_started is published for popout consumers (kiosk layout
  // reset + recording-state flip). The admin tab gets the new meeting
  // via the synchronous /api/meeting/start response and its periodic
  // /api/status poll; it does not need a named WS handler.
  'meeting_started',
]);
...
test('lifecycle resets all map to the same handler', () => {
  for (const slug of ['meeting_stopped', 'meeting_cancelled']) {
