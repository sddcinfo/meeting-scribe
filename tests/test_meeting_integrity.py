...
Run:
    ...
    MEETING_INTEGRITY_INCLUDE_PRIVATE=1 pytest tests/test_meeting_integrity.py
                                                    # include gitignored local meetings
"""
...
INCLUDE_PRIVATE_MEETINGS_ENV = "MEETING_INTEGRITY_INCLUDE_PRIVATE"
...
def _is_partial_meeting(meeting_dir: Path) -> bool:
    """Skip meetings that are obviously partial test scaffolds.

    A directory under ``meetings/`` without ``meta.json`` is either a
    diag/upload artifact (the slide-upload route creates ``slides/``
    eagerly) or a half-rolled-back legacy fixture. Either way, the
    quality validators will spuriously fail on it because they assume
    a proper meeting lifecycle. Filter at the discovery layer rather
    than threading "is this real?" checks through every validator.
    """
    return not (meeting_dir / "meta.json").is_file()
...
def analyze_meeting(meeting_dir: Path) -> MeetingReport:
    ...
    findings: list[Finding] = []
    # Structural validators run on every meeting regardless of state —
    # missing files in an incomplete meeting are still useful to surface.
    ...
    # Quality validators (orphan-micro-cluster, coverage-gap) only make
    # sense on a fully finalised meeting. An interrupted recording will
    # naturally have stray short clusters from interjections that the
    # finalisation pass usually consolidates. Running these checks on
    # state="recording" / state="cancelled" / state="unknown" produces
    # false positives every time a developer or test fixture creates a
    # short test meeting and the integrity-baseline diff turns red.
    # Scope them to ``complete`` (and the legacy ``stopped`` state for
    # pre-v1.3 meetings).
...
def collect_all_meetings() -> list[Path]:
    if not MEETINGS_DIR.exists():
        return []
    include_private = os.environ.get(INCLUDE_PRIVATE_MEETINGS_ENV)
    dirs = sorted(d for d in MEETINGS_DIR.iterdir() if d.is_dir() and not _is_partial_meeting(d))
    if include_private:
        return dirs
    visible: list[Path] = []
    for meeting_dir in dirs:
        try:
            ignored = (
                subprocess.run(
                    ["git", "check-ignore", "-q", str(meeting_dir)],
