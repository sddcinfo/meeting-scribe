...
def test_only_three_presets(page, presentation_server):
    """The popout offers four layouts: Translate, Presentation
    (translator), Triple stack, Translate + terminal. Removed:
    developer, fullstack, sidebyside, demo. The legacy "three"
    in the test name is preserved for git-history continuity."""
    _load(page, presentation_server["base_url"])
    ...
    assert presets["order"] == ["translate", "translator", "triple", "terminal"]
