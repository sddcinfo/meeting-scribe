...
def scribe_layout_server(tmp_path, monkeypatch) -> Generator[dict[str, Any]]:
    ...
    # `has_admin_session` in server_support/admin_guard.py reads the
    # signer off `state._terminal_cookie_signer` (a module-level
    # singleton, set by server.py at production boot). Tests skip
    # server.py's boot path, so we install the same singleton here —
    # otherwise every admin-only route in this fixture returns 401
    # regardless of whatever cookie playwright injects.
    prior_signer = getattr(runtime_state, "_terminal_cookie_signer", None)
    runtime_state._terminal_cookie_signer = cookie_signer

    app = FastAPI()
    # Bootstrap form (/admin/bootstrap) was deleted in v1.1 — the
    # current auth flow is `/auth` + wizard-password. We don't need any
    # of that here; the popout-layout surface only needs a valid admin
    # cookie, which we mint + inject directly via the playwright
    # context below (see _authorize_popout).
...
def _authorize_popout(page, srv) -> None:
    """Open `/?popout=view` with a pre-signed admin cookie.

    v1.1 retired `/admin/bootstrap`; the production auth flow is `/auth`
    + wizard-password. For the popout-layout tests we don't actually
    care which auth path placed the cookie — we just need a valid one
    so the admin-only WS routes (terminal) accept the connection. Mint
    one in-process via the same CookieSigner the server uses to verify
    it and inject via the playwright context.
    """
        ...
        [
            {
                ...
                "domain": "127.0.0.1",
                "path": "/",
                # Production sets secure=True; over plain HTTP in this
                # test harness we drop that so chromium will replay it.
                "secure": False,
                "httpOnly": True,
                "sameSite": "Strict",
            }
    ...
    page.goto(f"{srv['base_url']}/?popout=view", wait_until="domcontentloaded")
...
def test_popout_picker_renders_three_presets(page, scribe_layout_server):
    ...
    # PRESET_ORDER from static/js/popout-layout-presets.js is the
    # canonical source. The `terminal` preset (full-screen terminal,
    # no transcript/slides) joined the picker after this file was
    # originally written; keep this assertion in lockstep.
    assert state["options"] == ["translate", "translator", "triple", "terminal"]
...
def test_migration_from_terminal_visible_lands_on_triple(page, scribe_layout_server):
    ...
    """
    srv = scribe_layout_server
    # Land on the origin so we can write the localStorage key the
    # migration path consumes, then reload into the popout. (We can't
    # set localStorage before any navigation has happened; chromium
    # doesn't allow Storage access on `about:blank`.)
    _authorize_popout(page, srv)
