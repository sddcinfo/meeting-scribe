...
# ── Settings tabs ────────────────────────────────────────────────────
# Phase 2.6's token migration could silently break the settings drawer if
# any tab's component partial referenced a token name that wasn't
# renamed. These snapshots lock the rendered look of each tab so a
# future token-namespace drift gets caught at the cascade-resolution
# layer rather than in production.
...
def test_snapshot_settings_network_tab(page, vr_server):
    _viewport(page, 1280, 800)
    page.goto(f"{vr_server}/", wait_until="domcontentloaded")
    page.wait_for_function("() => !!window.audioPlayer", timeout=8000)
    _open_settings(page)
    # Network is the default-active tab; no click needed.
    _snapshot(page, "settings_network")


def test_snapshot_settings_audio_tab(page, vr_server):
    _viewport(page, 1280, 800)
    page.goto(f"{vr_server}/", wait_until="domcontentloaded")
    page.wait_for_function("() => !!window.audioPlayer", timeout=8000)
    _open_settings(page)
    ...
    page.wait_for_timeout(400)
    _snapshot(page, "settings_audio")


def test_snapshot_settings_credentials_tab(page, vr_server):
    _viewport(page, 1280, 800)
    page.goto(f"{vr_server}/", wait_until="domcontentloaded")
    page.wait_for_function("() => !!window.audioPlayer", timeout=8000)
    _open_settings(page)
    ...
    page.wait_for_timeout(400)
    _snapshot(page, "settings_credentials")


# ── Meetings panel slide-over ───────────────────────────────────────


def test_snapshot_meetings_panel_open(page, vr_server):
    _viewport(page, 1280, 800)
    page.goto(f"{vr_server}/", wait_until="domcontentloaded")
    page.wait_for_function("() => !!document.getElementById('btn-hamburger')", timeout=8000)
    page.click("#btn-hamburger")
    page.wait_for_timeout(800)
    _snapshot(page, "meetings_panel")


# ── Popout layout presets ───────────────────────────────────────────
# `popout_translate` + `popout_translator` baselines already exist via
# the original tests above; this commit adds the `triple` + `terminal`
# presets that landed during Phase 3 Tier 5.


def test_snapshot_popout_triple_preset(page, vr_server):
    _viewport(page, 1280, 720)
    page.goto(f"{vr_server}/?popout=view", wait_until="domcontentloaded")
    page.wait_for_function("() => !!window._gridRenderer", timeout=8000)
    page.evaluate(
        """() => {
            ...
            if (S) S.save({...S.DEFAULTS, preset: 'triple', customTree: null, ratiosByPreset: {}});
            const picker = document.getElementById('popout-layout-picker');
            if (picker) { picker.value = 'triple'; picker.dispatchEvent(new Event('change')); }
        }"""
    )
    page.wait_for_timeout(1200)
    # `triple` includes the live terminal panel — see the explanation
    # on test_snapshot_popout_terminal_preset for the wider tolerance.
    _snapshot(page, "popout_triple", byte_tolerance=2000)


def test_snapshot_popout_terminal_preset(page, vr_server):
    _viewport(page, 1280, 720)
    page.goto(f"{vr_server}/?popout=view", wait_until="domcontentloaded")
    page.wait_for_function("() => !!window._gridRenderer", timeout=8000)
    page.evaluate(
        """() => {
            const S = window.PopoutLayoutStorage;
            if (S) S.save({...S.DEFAULTS, preset: 'terminal', customTree: null, ratiosByPreset: {}});
            const picker = document.getElementById('popout-layout-picker');
            if (picker) { picker.value = 'terminal'; picker.dispatchEvent(new Event('change')); }
        }"""
    )
    page.wait_for_timeout(1200)
    # Terminal panel is intrinsically dynamic (xterm.js cursor + lazy
    # connection state); use a wider tolerance to catch layout drift
    # without false-positiving on cursor-position noise.
    _snapshot(page, "popout_terminal", byte_tolerance=2000)


# ── Mobile viewports ────────────────────────────────────────────────
# Phase 2 promised viewport coverage; the original snapshot suite is
# desktop-only. These add the canonical mobile viewports from
# static/TESTING_VIEWPORTS.md (iPhone 16 / iPhone Max / iPad Pro 11).
# Token-resolution regressions tend to surface first on the smallest
# viewport because the responsive cascade pulls in extra rules.


def test_snapshot_admin_landing_iphone_16(page, vr_server):
    _viewport(page, 390, 844)
    page.goto(f"{vr_server}/", wait_until="domcontentloaded")
    page.wait_for_timeout(800)
    _snapshot(page, "admin_landing_iphone_16", byte_tolerance=400)


def test_snapshot_admin_landing_ipad_pro_11(page, vr_server):
    _viewport(page, 1194, 834)
    page.goto(f"{vr_server}/", wait_until="domcontentloaded")
    page.wait_for_timeout(800)
    _snapshot(page, "admin_landing_ipad_pro_11", byte_tolerance=400)


def test_snapshot_popout_translate_iphone_16(page, vr_server):
    _viewport(page, 390, 844)
    page.goto(f"{vr_server}/?popout=view", wait_until="domcontentloaded")
    page.wait_for_function("() => !!window._gridRenderer", timeout=8000)
    page.wait_for_timeout(1000)
    _snapshot(page, "popout_translate_iphone_16", byte_tolerance=400)
