...
The current registry has four presets (see
``static/js/popout-layout-presets.js``):
  ...
  * ``triple``     — ``transcript`` over (``slides`` over ``terminal``).
  * ``terminal``   — ``terminal`` over ``transcript`` (debug/demo console).
"""
...
def test_preset_registry_lists_three_presets_in_order(page, unit_server):
    ...
    assert order == ["translate", "translator", "triple", "terminal"]
...
def test_only_triple_carries_terminal(page, unit_server):
    ...
    use to decide last-term vs last-no-term memory. Triple AND the
    Translate+terminal console both include a terminal pane; the two
    transcript-only presets don't.
    """
    _load(page, unit_server["base_url"])
    flags = page.evaluate(
        """() => {
            ...
            return {
                ...
                terminal: P.hasTerminal('terminal'),
            };
        }"""
    )
    assert flags == {
