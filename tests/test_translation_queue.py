...
class TestTranslationTargetRouting:
    ...
    @pytest.mark.asyncio
    async def test_wrong_script_translation_retries_once(self):
        results = []
        ...
        class MockBackend:
            ...
            async def translate(self, text, source_language, target_language, **kwargs):
                self.calls += 1
                assert target_language == "en"
                return "これは英語ではありません" if self.calls == 1 else "This is English."
        ...
        assert len(done_events) == 1
        assert done_events[0].translation.text == "This is English."
