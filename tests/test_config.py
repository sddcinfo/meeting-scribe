...
class TestGB10Profile:
    ...
    def test_gb10_profile_tts_url(self):
        cfg = ServerConfig.from_profile("gb10")
        assert cfg.tts_vllm_url == "http://localhost:8002"
...
class TestCaptiveHttpPort:
    """``captive_http_port`` env-var resolution: new wins, legacy
    falls through with a deprecation warning, invalid → default."""

    def test_default_is_80(self, monkeypatch):
        monkeypatch.delenv("SCRIBE_CAPTIVE_HTTP_PORT", raising=False)
        monkeypatch.delenv("SCRIBE_GUEST_PORT", raising=False)
        cfg = ServerConfig.from_env()
        assert cfg.captive_http_port == 80

    def test_new_env_wins(self, monkeypatch):
        monkeypatch.setenv("SCRIBE_CAPTIVE_HTTP_PORT", "8888")
        monkeypatch.delenv("SCRIBE_GUEST_PORT", raising=False)
        cfg = ServerConfig.from_env()
        assert cfg.captive_http_port == 8888

    def test_legacy_env_falls_through(self, monkeypatch, caplog):
        import logging

        monkeypatch.delenv("SCRIBE_CAPTIVE_HTTP_PORT", raising=False)
        monkeypatch.setenv("SCRIBE_GUEST_PORT", "7777")
        with caplog.at_level(logging.WARNING, logger="meeting_scribe.config"):
            cfg = ServerConfig.from_env()
        assert cfg.captive_http_port == 7777
        assert any("SCRIBE_GUEST_PORT is deprecated" in record.message for record in caplog.records)

    def test_new_env_wins_over_legacy(self, monkeypatch, caplog):
        """Both set → new value wins; legacy still warns."""
        import logging

        monkeypatch.setenv("SCRIBE_CAPTIVE_HTTP_PORT", "5555")
        monkeypatch.setenv("SCRIBE_GUEST_PORT", "9999")
        with caplog.at_level(logging.WARNING, logger="meeting_scribe.config"):
            cfg = ServerConfig.from_env()
        assert cfg.captive_http_port == 5555
        assert any("SCRIBE_GUEST_PORT is deprecated" in record.message for record in caplog.records)

    def test_invalid_value_falls_back(self, monkeypatch):
        """Non-numeric value → default 80, no crash."""
        monkeypatch.setenv("SCRIBE_CAPTIVE_HTTP_PORT", "not-a-port")
        monkeypatch.delenv("SCRIBE_GUEST_PORT", raising=False)
        cfg = ServerConfig.from_env()
        assert cfg.captive_http_port == 80
