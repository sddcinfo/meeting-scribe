...
def test_create_session_picks_up_persisted_voice_mode(monkeypatch):
    """New audio-out clients inherit ``tts_voice_mode`` from settings.

    Contract: whatever the operator persisted via /api/admin/settings
    (``tts_voice_mode = "studio" | "cloned"``) is what every new WS
    listener sees as ``pref.voice_mode`` on connect. Per-session
    ``set_voice`` overrides still win for that one listener; this
    test only locks down the default-on-handshake path.
    """
    ...
    monkeypatch.setattr(settings_store, "_load_settings_override", _fake_load)

    overrides["tts_voice_mode"] = "cloned"
    ws_a = FakeWs()
    try:
        pref_a = audio_output._create_audio_out_session(ws_a)
        assert pref_a.voice_mode == "cloned"
    finally:
        state._audio_out_prefs.pop(ws_a, None)

    overrides["tts_voice_mode"] = "studio"
    ws_b = FakeWs()
    try:
        pref_b = audio_output._create_audio_out_session(ws_b)
        assert pref_b.voice_mode == "studio"
    finally:
        state._audio_out_prefs.pop(ws_b, None)

    # Unknown / missing override falls back to "studio".
    overrides.pop("tts_voice_mode", None)
    ws_c = FakeWs()
    try:
        pref_c = audio_output._create_audio_out_session(ws_c)
        assert pref_c.voice_mode == "studio"
    finally:
        state._audio_out_prefs.pop(ws_c, None)
