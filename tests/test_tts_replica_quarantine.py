...
def test_app_timeout_quarantines_only_timed_out_replica():
    """A worker-level timeout parks the selected URL, not the whole pool."""
    b = _backend()

    b.mark_synthesis_timeout(URL_A, 8.0)

    assert URL_A in b._quarantined
    assert URL_B not in b._quarantined
    assert not b.degraded
    assert [b._next_url() for _ in range(3)] == [URL_B, URL_B, URL_B]


def test_app_timeout_marks_degraded_when_all_replicas_quarantined():
    """The backend is globally degraded only when no healthy replica remains."""
    b = _backend()

    b.mark_synthesis_timeout(URL_A, 8.0)
    b.mark_synthesis_timeout(URL_B, 8.0)

    assert b.degraded


def test_choose_url_for_request_obeys_quarantine():
    """Worker preselection uses the same quarantine-aware routing as synth."""
    b = _backend()
    b.mark_synthesis_timeout(URL_A, 8.0)

    assert b.choose_url_for_request() == URL_B


def test_reset_failure_state_after_restart_restores_pool():
    """A container restart is the explicit recovery point for CUDA failures."""
    b = _backend()
    b.mark_synthesis_timeout(URL_A, 8.0)
    b.mark_synthesis_timeout(URL_B, 8.0)
    ...
    asyncio.run(b.reset_failure_state_after_restart())

    assert not b.degraded
    assert b._url_failures == {}
    assert b._quarantined == {}
    assert b.last_error is None
    assert {b.choose_url_for_request(), b.choose_url_for_request()} == {URL_A, URL_B}
    assert b._in_restart_grace()


def test_restart_grace_failure_does_not_redegrade_pool():
    """Connection failures while the restarted container warms do not thrash."""
    b = _backend()
    asyncio.run(b.reset_failure_state_after_restart())
    ...
    b._http_client = _FailingClient()

    async def _consume():
        return [
            chunk
            async for chunk in b.synthesize_stream(
                "hello", "English", studio_voice="ryan", target_url=URL_A
            )
        ]
