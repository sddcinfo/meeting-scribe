...
(function () {
  ...
  function init() {
    ...
    window.addEventListener('meeting-scribe:close-diagnostics', closePanel);
    window.closeDiagnosticsPanel = closePanel;
  ...
  function open() {
    ...
    window.dispatchEvent(new CustomEvent('meeting-scribe:close-settings'));
  ...
  async function pollHealth() {
    ...
    // GPU topology has its own admin gate + cache; fetch in parallel so a
    // slow nvidia-smi never blocks the rest of the panel.
    try {
      const r = await adminFetch('/api/gpu/topology');
      const topo = await r.json();
      renderGpuTopology(topo);
    } catch (e) {
      if (e.message !== 'forbidden') console.warn('[diag] gpu poll failed', e);
    }
  }
  ...
  function renderHealth(data) {
    ...
    // System (CPU/RAM/load — GPU has its own section now)
  ...
  // ── GPU section ────────────────────────────────────────────────────
  //
  // Renders /api/gpu/topology (proxied to `sddc gpu top --json`):
  //   • card-level summary  → diag-gpu-card-grid (stat cards)
  //   • process table       → diag-gpu-process-tbody (one row per PID)
  //   • engine table        → diag-gpu-engine-tbody (one row per vLLM /metrics)
  // Rates (tok/s, MFU%) populate from the second poll onwards because
  // the underlying counters need a delta sample.
  function renderGpuTopology(topo) {
    const banner = $('diag-gpu-error');
    if (banner) {
      banner.hidden = !topo?.error;
      banner.textContent = topo?.error ? `GPU snapshot unavailable: ${topo.error}` : '';
    }
    if (!topo || topo.error) return;

    const card = topo.card || {};
    const cardGrid = $('diag-gpu-card-grid');
    if (cardGrid) {
      const pwr = card.power_w != null
        ? `${Math.round(card.power_w)}W${card.power_cap_w ? ` / ${Math.round(card.power_cap_w)}W` : ''}`
        : '—';
      const throttle = card.throttle_reasons || '—';
      const throttleLevel = (throttle === 'none' || throttle === 'n/a') ? 'ok' : 'warn';
      cardGrid.innerHTML = [
        statCardHTML('SM', `${card.sm_pct ?? 0}%`, gauge(card.sm_pct)),
        statCardHTML('Mem I/O', `${card.mem_io_pct ?? 0}%`, gauge(card.mem_io_pct)),
        statCardHTML('VRAM', `${fmtMb(card.vram_used_mb)} / ${fmtMb(card.vram_total_mb)}`, gauge((card.vram_used_mb / card.vram_total_mb) * 100)),
        statCardHTML('Power', pwr),
        statCardHTML('Temp', card.temp_c != null ? `${card.temp_c}°C` : '—', card.temp_c >= 80 ? 'warn' : 'ok'),
        statCardHTML('Throttle', throttle, throttleLevel),
      ].join('');
    }

    const procBody = $('diag-gpu-process-tbody');
    if (procBody) {
      const procs = topo.processes || [];
      if (!procs.length) {
        procBody.innerHTML = '<tr><td colspan="5" class="diag-empty">No GPU processes.</td></tr>';
      } else {
        procBody.innerHTML = procs.map((p) => `
          <tr>
            <td><span class="diag-gpu-role">${esc(p.role || '?')}</span></td>
            <td><code>${esc(p.container || '-')}</code></td>
            <td class="diag-num">${esc(String(p.pid))}</td>
            <td class="diag-num">${p.sm_pct ?? 0}%</td>
            <td class="diag-num">${fmtMb(p.vram_mb)}</td>
          </tr>
        `).join('');
      }
    }

    const engBody = $('diag-gpu-engine-tbody');
    if (engBody) {
      const engines = topo.engines || [];
      if (!engines.length) {
        engBody.innerHTML = '<tr><td colspan="9" class="diag-empty">No vLLM engines configured.</td></tr>';
      } else {
        engBody.innerHTML = engines.map(engineRowHTML).join('');
      }
    }
  }
  ...
  function engineRowHTML(e) {
    if (!e.reachable) {
      return `
        <tr data-level="warn">
          <td><span class="diag-gpu-role">${esc(e.name || '?')}</span></td>
          <td colspan="8"><em>unreachable: ${esc(e.error || '?')}</em></td>
        </tr>`;
    }
    ...
    return `
      <tr>
        <td><span class="diag-gpu-role">${esc(e.name || '?')}</span></td>
        <td><code title="${esc(e.url || '')}">${esc(e.model || '?')}</code></td>
        <td class="diag-num">${e.running ?? 0}</td>
        <td class="diag-num">${e.waiting ?? 0}</td>
        <td class="diag-num">${(e.kv_pct ?? 0).toFixed(1)}%</td>
        <td class="diag-num">${fmt(e.gen_tok_per_s)}</td>
        <td class="diag-num">${e.ttft_p50_ms != null ? Math.round(e.ttft_p50_ms) + 'ms' : '—'}</td>
        <td class="diag-num">${e.tpot_p50_ms != null ? e.tpot_p50_ms.toFixed(1) + 'ms' : '—'}</td>
        <td class="diag-num">${fmt(e.mfu_pct, '%')}</td>
      </tr>
    `;
  }
