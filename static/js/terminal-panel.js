 ...
 *
 * Exports `window.TerminalPanel`. The popout SPA loader instantiates
 * one on first toggle and keeps the same instance around.
 *
 * Design notes:
 *   · Auth is a two-factor handshake: admin cookie (from /auth)
...
(() => {
  ...
  // sessionStorage key — per-tab, NOT per-browser. Each tab gets its own
  // tmux session so resize-window calls don't clip a sibling tab.
  const SS = {
    tabSession: 'terminal_tab_session',
  };

  /**
   * Resolve the per-tab tmux session id with a three-layer policy:
   *
   * 1. URL hash `#tabId=<value>` always wins. Used by the popout flow:
   *    the inline admin tab opens the popout with `noopener` and a fresh
   *    tabId in the hash; even if the browser somehow leaked the
   *    opener's sessionStorage, this layer overwrites it BEFORE attach.
   * 2. sessionStorage — survives a reload-this-tab so the user lands
   *    back in the same shell. Cleared when the tab is closed.
   * 3. Fresh `crypto.randomUUID()` minted on first call and persisted
   *    in sessionStorage.
   *
   * Validation regex matches the server-side TMUX_NAME_RE (≤40 chars,
   * alphanumeric + `_-`). Anything else is treated as missing.
   */
  function _resolveTabSessionId() {
    const VALID = /^[A-Za-z0-9_-]{1,40}$/;
    // Layer 1 — URL hash (location.hash includes the leading '#').
    try {
      const rawHash = (location.hash || '').replace(/^#/, '');
      const hashTabId = new URLSearchParams(rawHash).get('tabId');
      if (hashTabId && VALID.test(hashTabId)) {
        sessionStorage.setItem(SS.tabSession, hashTabId);
        return hashTabId;
      }
    } catch { /* sessionStorage unavailable in some sandboxed contexts */ }
    // Layer 2 — per-tab persistent id surviving reload of THIS tab.
    let token;
    try { token = sessionStorage.getItem(SS.tabSession); } catch { token = null; }
    if (token && VALID.test(token)) return token;
    // Layer 3 — fresh UUID. crypto.randomUUID returns 36 chars with
    // dashes; strip dashes to fit the 40-char limit comfortably and
    // stay inside the alphanumeric+_- charset.
    const fresh = (crypto.randomUUID && crypto.randomUUID().replace(/-/g, ''))
              || ('t' + Math.random().toString(36).slice(2)
                       + Date.now().toString(36)).slice(0, 40);
    try { sessionStorage.setItem(SS.tabSession, fresh); } catch { /* */ }
    return fresh;
  }
  ...
  class TerminalPanel {
    constructor({ apiBase, wsBase } = {}) {
      ...
      // Per-tab tmux session id (see _resolveTabSessionId for the
      // hash → sessionStorage → fresh-UUID policy). Falls back to the
      // legacy localStorage value or the global "scribe" name only when
      // sessionStorage is unavailable, so manual operators using
      // `tmux -L scribe attach -t <name>` keep working.
      try {
        this._tmuxSession = _resolveTabSessionId();
      } catch {
        this._tmuxSession = localStorage.getItem(LS.sessionName) || 'scribe';
      }
      this._resizeObs = null;
      this._resizeTimer = null;            // setTimeout id for debounced server msg
      this._refitRafPending = false;       // rAF dedup for the local xterm.fit()
    ...
    async mount() {
      ...
      // NOTE: do NOT auto-show from inside mount(). The caller (the
      // popout SPA / control-bar toggle) owns the auto-restore
      // decision. Auto-showing here caused show() to run twice in
      // quick succession, which raced _connect() — two WebSockets
      // attached to the same tmux session → shell echo reached
      // xterm twice → every keystroke rendered doubled.
    }
    ...
    _buildDom() {
      ...
      this._authOverlay = el('div', { class: 'term-auth-overlay', hidden: '' }, [
        el('div', { class: 'term-auth-card' }, [
          ...
          el('div', { class: 'term-auth-body', text: 'Sign in as admin to mint a signed cookie for this device.' }),
          el('a', {
            class: 'term-auth-cta',
            href: '/auth',
            target: '_blank',
    ...
    _installResizeObserver() {
      // ResizeObserver fires on every layout cycle the body's box changes
      // through — that's many frames per second during a continuous
      // window-corner drag. We want xterm's cell grid to track those
      // changes live (so the user sees the terminal reflow under their
      // cursor, not snap at the end). Server-side tmux resize-window
      // is too slow for 60fps, so the *message* path stays debounced.
    ...
    _installShortcuts() {
      ...
      // the admin SPA boot because xterm has focus when the panel is
      // open.
    }

    _scheduleRefit() {
      // 1) Refit xterm locally on the next animation frame. Cheap (it's a
      //    DOM measurement + xterm grid update; no network), so we run it
      //    every frame during a drag — but rAF-dedup so a burst of
      //    ResizeObserver ticks within one frame coalesces to one fit().
      if (!this._refitRafPending) {
        this._refitRafPending = true;
        requestAnimationFrame(() => {
          this._refitRafPending = false;
          if (!this._fit || !this._term) return;
          try {
            this._fit.fit();
            const cols = this._term.cols;
            const rows = this._term.rows;
            if (this._dimsSpan) this._dimsSpan.textContent = `${cols}×${rows}`;
            this._queueServerResize(cols, rows);
          } catch {}
        });
      }
    }

    _queueServerResize(cols, rows) {
      // 2) Debounce the server message — TIOCSWINSZ + tmux resize-window
      //    take ~ms-to-tens-of-ms server-side and serialize on the WS
      //    receive loop, so firing one per frame would back the loop up
      //    fast. We send only the final size after the user pauses, but
      //    we also skip the send entirely if the local fit ended at the
      //    same cols/rows we last reported.
      if (cols === this._lastSentCols && rows === this._lastSentRows) return;
      this._pendingCols = cols;
      this._pendingRows = rows;
      clearTimeout(this._resizeTimer);
      this._resizeTimer = setTimeout(() => {
        this._sendText({ type: 'resize', cols: this._pendingCols, rows: this._pendingRows });
        this._lastSentCols = this._pendingCols;
        this._lastSentRows = this._pendingRows;
      }, RESIZE_DEBOUNCE_MS);
    }
    ...
    _onWsOpen(ticket) {
      ...
      // Treat the attach as having "sent" these dims — otherwise the
      // first _scheduleRefit after WS open would always re-broadcast
      // the same numbers as a resize.
      this._lastSentCols = this._term.cols;
      this._lastSentRows = this._term.rows;
        ...
        case 'attached':
          ...
          if (this._term) {
            if (this._term.cols !== msg.cols || this._term.rows !== msg.rows) {
              ...
              this._lastSentCols = msg.cols;
              this._lastSentRows = msg.rows;
            }
          }
          break;
        case 'status':
    ...
    // ── Session picker ───────────────────────────────────────────
    async _promptSession() {
      const name = await window.promptDialog(
        'tmux session name',
        'letters, digits, _ or -, max 32 chars',
        { defaultValue: this._tmuxSession, confirmText: 'Switch' }
      );
      if (!name) return;
      if (!this.setTmuxSession(name)) {
        await window.alertDialog('Invalid session name', String(name));
      }
    }
