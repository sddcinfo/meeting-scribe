...
// The admin SPA boot wires real DOM / sessionStorage / fetch /
// AudioPipeline primitives; tests wire in-memory spies.
//
// Keeping the state machine self-contained here lets node --test
// exercise every branch in isolation without booting the full SPA
// surface.
...
export function createReconciler(deps) {
  ...
  async function enterLiveMeetingMode(meetingId, opts = {}) {
    const { resetStore: shouldReset = true, forceRecorder = false } = opts;
    ...
    try {
      ...
      } else if (_tabOwnsOrAcquiringTransport()) {
  ...
  async function takeoverRecorder(meetingId) {
    ...
    await enterLiveMeetingMode(meetingId, { resetStore: true, forceRecorder: true });
  }
  ...
  async function reconcile(status) {
    ...
    // ── ``body.recording`` tracks server state authoritatively ────
    // The button at #btn-record decides start-vs-stop by reading
    // ``body.classList.contains('recording')``. Driving the class
    // from ``status.meeting.state`` here is the single ownership
    // path for that signal: every other writer (the start-flow add
    // in recording-lifecycle.js, the cleanup remove in
    // ``onFinalizeCleanup`` below) is reached via ``reconcile`` so
    // this stays the canonical signal even across a page reload
    // mid-meeting.
    if (isRecording && !bodyRecording) {
      doc.body.classList.add("recording");
    } else if (!isRecording && bodyRecording) {
      // The remove is delegated to ``onFinalizeCleanup`` below so
      // the full teardown chain (audio ws close, view-only detach,
      // also ``meeting-active`` and ``starting`` removed) fires
      // atomically. Don't ``remove("recording")`` here in
      // isolation — leave the class until cleanup runs.
    }
