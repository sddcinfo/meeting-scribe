...
export class SegmentStore {
  ...
  ingest(event) {
    ...
    // render paths that don't know about the multi-target shape
    // still work without a display-language picker.
    //
    ...
    if (newTr) {
      ...
      // ``newLang`` arrives from a WebSocket event and is used as a
      // property key — reject the prototype-pollution sentinels and
      // anything that doesn't shape like a BCP 47 language tag (2–10
      // ASCII letters/digits/dashes). Closes CodeQL
      // js/remote-property-injection.
      const _isLangKey = (k) =>
        typeof k === 'string' &&
        k !== '__proto__' && k !== 'constructor' && k !== 'prototype' &&
        /^[A-Za-z][A-Za-z0-9-]{0,9}$/.test(k);
      if (newLang && _isLangKey(newLang)) {
