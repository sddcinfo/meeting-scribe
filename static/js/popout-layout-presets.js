...
(function () {
  ...
  const triple = {
    slug: 'triple',
    label: 'Triple stack',
    description: 'Translation on top; slides and terminal side-by-side below.',
    tree: split(
      'v', 0.45,
      leaf('transcript'),
      split(
  ...
  // Translate + terminal (no slides). Standalone popout window for
  // operators who want to drag the terminal to a second monitor while
  // keeping the transcript on the primary. Vertical split: transcript
  // on top (the primary content), terminal docked below — matches the
  // operator's mental model of the terminal as a debug/demo footer
  // beneath the live transcript.
  const terminalConsole = {
    slug: 'terminal',
    label: 'Translate + terminal',
    description: 'Translation on top, terminal below — debug/demo console.',
    tree: split(
      'v', 0.6,
      leaf('transcript'),
      leaf('terminal'),
  ...
  const PRESETS = { translate, translator, triple, terminal: terminalConsole };
  const PRESET_ORDER = ['translate', 'translator', 'triple', 'terminal'];
