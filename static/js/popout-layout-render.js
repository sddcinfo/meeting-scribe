...
(function () {
  ...
  async function _buildSplit(node, onRatioCommit, path) {
    ...
    window.PopoutLayoutResizer.createResizer(gutter, splitEl, {
      dir: node.dir,
      // rAF-throttled — fires up to ~60×/s during drag so the terminal
      // refits its xterm grid + sends the new cols/rows to the server
      // (which TIOCSWINSZ-resizes the tmux client) WHILE the user
      // drags, not just on mouseup. ResizeObserver alone catches most
      // cases via flex reflow, but the inline _body height set at
      // panel construction has burned us before — explicitly poking
      // the registry is cheap insurance and makes the resize feel
      // tracked rather than snapping at commit.
      onUpdate: () => {
  ...
  function install(rootEl, state, availability = {}, opts = {}) {
    ...
    // Server-authoritative layout pushes (e.g. meeting_started reset,
    // or the laptop admin changing the preset). Storage's WS subscriber
    // dispatches this CustomEvent; we re-render against the supplied
    // state object so the kiosk mirror tracks the admin pop-out.
    window.addEventListener('popout-layout:remote-apply', (e) => {
      const next = e && e.detail && e.detail.layout;
      if (!next || typeof next !== 'object') return;
      _state = next;
      if (_rootEl) renderLayout(_rootEl, _state);
    });
