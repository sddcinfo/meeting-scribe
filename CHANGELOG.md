...
### Audio playback (2026-05-07)

- **HTTP `Range` support on `/api/meetings/<id>/audio`.** The endpoint
  declared `Accept-Ranges: bytes` but ignored every `Range:` header,
  so seek-driven `<audio>` fetches on a 1+ hour meeting (~200 MB PCM)
  re-downloaded the full file on every browser-side seek. Now parses
  `bytes=N-M` (incl. open-ended and suffix forms), returns
  `206 Partial Content` with `Content-Range`, and streams from disk
  in 64 KiB chunks instead of loading the full PCM into memory. Per-
  request peak memory drops from ~200 MB to 64 KiB; long-meeting seek
  starts playing immediately.
- **`AudioPlayer` `play()`/`load()` race fix.** `audio.src = X;
  audio.play()` was the wrong pattern — setting `src` kicks off an
  async load that the immediate `play()` raced. If anything else set
  `src` before the first `play()` promise resolved (segment switch,
  TTS auto-play overlap), the promise rejected with AbortError and
  the UI silently stalled. All call sites now route through
  `_setSrcAndPlay` / `_safePlay`, which swallow the expected
  AbortError. `loadedmetadata` / `error` / `stalled` / `waiting`
  handlers wired up for visibility into real failures.
- **150 ms lead-in on segment-jump clicks** (timeline lane block,
  speaker-modal "play this segment", compact-block transcript row).
  ASR alignment locks `start_ms` to the strongest spectral cue,
  which usually lands 50–200 ms after the breath / soft-consonant
  onset; without a lead-in the leading partial syllable is clipped.
- **Empty-track diarize-lane clicks snap to nearest block.** Block
  widths are tiny on long meetings (CSS minimum is 0.15 % of the
  timeline ≈ 9.6 s), so most "clicked the lane" clicks landed
  between blocks and seeked to an arbitrary % of duration. Now snaps
  to the nearest block in that lane (with the same 150 ms lead-in)
  so the click always resolves to a transcript-segment onset.
- **`debug_audio_timing` runtime knob.** Hot-reloadable knob
  (`meeting-scribe config set debug_audio_timing true`) makes the
  request-timing middleware log every `/api/meetings/<id>/audio`
  request with TTFB, status, content-type, content-length, the
  request `Range` header, and querystring — regardless of the
  500 ms `SLOW HTTP` threshold. Off by default.
- **`benchmarks/loaded_playback_check.py`** rewritten to assert
  authenticated audio responses (`Content-Type: audio/wav`, body
  `> 1024 B`, no redirect-following). The previous version followed
  302 → /auth and silently passed on the HTML login page.

### vLLM recipe (2026-05-07)

- **`max_num_seqs` 8 → 4, `kv_cache_dtype` auto → fp8** on the
  `qwen3.6-35b-a3b-fp8` recipe. The `vllm_kv_ab_2026-05-07` campaign
  showed peak demand under the perf-harness workload is 3 slots;
  `max_num_seqs=4` holds the same TTFT envelope as 8 with one slot
  of headroom, while `max_num_seqs=2` forces 117 preempts and an 8×
  coding TTFT p95 regression. fp8 KV cache no longer reproduces
  vllm#26646 on `vllm/vllm-openai:latest` (which logs
  `[SM121] FP8 fix applied: CUTLASS block-FP8 disabled, Triton
  fallback enabled`); it boots cleanly, runs the perf harness with
  zero errors, is BLEU-better than `auto` (+3.26 / +2.16 vs. C0),
  and reliably handles the 782-event long meeting that the `auto`
  path consistently timed out on. Full data:
  `benchmarks/results/vllm_kv_ab_2026-05-07/RESULT.md`.
- **`benchmarks/README.md` recipe pointer fix.** The Hardware section
  cited the retired `qwen3.6-fp8-nightly.yaml` and the `:nightly`
  image tag; both updated to the canonical
  `qwen3.6-35b-a3b-fp8.yaml` + `:latest`.

### Tooling (2026-05-07)

- **`CONTRIBUTING.md` hook path** corrected from `scripts/hooks/`
  to `.githooks` (matches `pre-commit` / `pre-push` install paths
  used by `scripts/ci_local.py`).
- **Reprocess regression fix** — full-reprocess silently failed to
  regenerate `timeline.json` + `speaker_lanes.json` because the
  speaker-data phase was importing `_generate_speaker_data` /
  `_generate_timeline` off `meeting_scribe.server`, where they no
  longer live (moved to `server_support.meeting_artifacts` in the
  v2.0.0 refactor). The surrounding `try/except Exception` swallowed
  the resulting AttributeError, so every full-reprocess since the
  refactor left the meeting with a fresh journal.jsonl whose
  segment_ids didn't match the stale lanes — clicking the diarize
  timeline became a no-op. Symptom-spotted by user on meeting
  `e6b1a2a6-...`.
- **`scripts/audit_dynamic_imports.py`** — static-AST guard so the
  `importlib.import_module(M); m.X` pattern can't silently break
  again. Scans every reference, asserts the attribute exists in M
  today. Currently 0 unique references in `src/`.

### Performance (2026-05-07)

- **`vm.swappiness=10`** baked into the install path. New file
  `provisioning/sysctl/60-meeting-scribe.conf` is dropped into
  `/etc/sysctl.d/` by `bootstrap.sh` (new section §5.5) and applied
  via `sysctl --system`. Default Ubuntu value is 60, which biases
  the kernel toward swapping out anonymous pages (the live Python
  process, vLLM working set) to keep page cache resident — exactly
  backwards for an interactive transcription box. Symptom this
  fixes: "audio playback goes choppy when reprocess is running" —
  caused by the scribe Python process getting paged out under
  reprocess memory pressure, then having to fault back in on every
  audio request. Idempotent on re-run; survives reboot. Applied to
  the dev box manually 2026-05-07; customer GB10s pick it up on
  next bootstrap run.
